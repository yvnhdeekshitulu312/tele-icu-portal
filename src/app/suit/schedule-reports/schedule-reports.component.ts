...
export class ScheduleReportsComponent extends BaseComponent implements OnInit {
    data: any
    ...
    fetchResourceTypes() {
        const url = this.us.getApiUrl(ScheduleReports.FetchScheduleServicesMappedResourceTypes, {
        ...
        this.us.get(url).subscribe((response: any) => {
            if (response.Code === 200) {
                this.resourceTypeList = response.FetchScheduleServicesMappedResourceTypesDataList;
                this
            }
        });
    }
    ...
    onSearchResourceName(event: any) {
        ...
        if (event.target.value.length >= 3) {
            const url = this.us.getApiUrl(ScheduleReports.FetchPhysiotheraphyUsers, {
    ...
    fetchReports() {
        this.errorMessages = [];
        // if (!this.reportsForm.get('resourceName')?.value) {
        //     this.errorMessages.push('Please Select Resource Name');
        // }
        ...
        const url = this.us.getApiUrl(ScheduleReports.FetchResourceCalenderPhy, {
    ...
    fetchReportsPrint() {
        ...
        const FromDate = this.datepipe.transform(this.reportsForm.value['FromDate'], "dd-MMM-yyyy")?.toString();
        const ToDate = this.datepipe.transform(this.reportsForm.value['ToDate'], "dd-MMM-yyyy")?.toString();
        const url = this.us.getApiUrl(ScheduleReports.FetchResourceCalenderPhyPrint, {
            Resource: this.reportsForm.get('resourceName')?.value == '' ? 0 : this.reportsForm.get('resourceName')?.value.ProcedureID,
            SSN: this.reportsForm.get('SSN')?.value ? this.reportsForm.get('SSN')?.value : 0,
            FromDate,
            ToDate,
    ...
    updateAppointment() {
        ...
        this.us.post(ScheduleReports.UpdatePatientAppointmentConfirmStatus, payload).subscribe((response) => {
            if (response.Code == 200) {
                $('#saveMsg').modal('show');
                this.fetchReports();
            }
...
export const ScheduleReports = {
    FetchScheduleServicesMappedResourceTypes: 'FetchScheduleServicesMappedResourceTypes?UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
    FetchPhysiotheraphyUsers: 'FetchPhysiotheraphyUsers?Filter=${Filter}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
    FetchResourceCalenderPhy: 'FetchResourceCalenderPhy?Resource=${Resource}&SSN=${SSN}&FromDate=${FromDate}&ToDate=${ToDate}&Status=${Status}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
    FetchResourceCalenderPhyPrint: 'FetchResourceCalenderPhyPrint?Resource=${Resource}&SSN=${SSN}&FromDate=${FromDate}&ToDate=${ToDate}&Status=${Status}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
