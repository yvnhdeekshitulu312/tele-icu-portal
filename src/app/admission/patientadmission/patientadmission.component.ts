...
@Component({
  ...
  styleUrls: ['./patientadmission.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class PatientadmissionComponent extends BaseComponent implements OnInit {
  ...
  showCancelAdmission: boolean = false;
  ...
  FetchAdmissionWardNBedsList: any;
  showRemarksValidation: boolean = false;
  aprrovalRequests: any[] = [];
  aprrovalRequestDetails: any[] = [];
  facility: any;
  patientPrevSurgeryReqs: any = [];
  allServiceItemsList: any = [];
  itemPrice: any = "";
  itemPriceList: any = [];
  selectedServiceId: string = '0';
  isFromAdmissionWorklist: boolean = false;
  serviceInfoeligibleBedTypeList: any = [];
  isSSNDisabled: boolean = false;

  constructor(private router: Router, private us: UtilityService, private service: PatientadmissionService,
    private config: ConfigService, public formBuilder: FormBuilder, public datepipe: DatePipe,
    private modalSvc: NgbModal, private modalService: GenericModalBuilder, private biometricService: BiometricService) {
    super();
    this.service.param = {
      ...this.service.param,
    ...
    this.patientAdmissionForm = this.formBuilder.group({
      ...
      ConsultantName: [''],
      ExptDisChargeDate: ['', Validators.required],
      IsRefDocExternal: [''],
      ExRefDocID: [''],
      RefDocID: [''],
      QualificationID: [''],
  ...
  ngOnInit(): void {
    ...
    const fromAdmissionWorklist = sessionStorage.getItem("fromAdmissionWorklist") ? true : false;
    if (fromAdmissionWorklist) {
      this.isFromAdmissionWorklist = true;
      this.fetchPatientDetails(JSON.parse(sessionStorage.getItem("fromAdmissionWorklist") || '{}').SSN, '0', '0')
    }
    if (this.selectedPatientForAdmission.length != undefined || this.selectedPatientForAdmission.PatientID != undefined) {
      ...
      this.patientAdmissionForm.patchValue({
        ...
        ConsultantName: this.selectedPatientForAdmission.DoctorName,
      ...
      this.patientAdmissionForm.patchValue({
        ...
        ExptDisChargeDate: (this.selectedPatientForAdmission.ExptDisChargeDate != null && this.selectedPatientForAdmission.ExptDisChargeDate != '') ? new Date(this.selectedPatientForAdmission.ExptDisChargeDate) : '',
    ...
    // this.akuWrklstPatient = JSON.parse(sessionStorage.getItem("otpatient") ?? '{}');
    // if(this.fromAkuWorklist === 'true' && this.selectedPatientForAdmission.IPAdmissionID=='') {
  ...
  FetchPatientEMRIP() {
    ...
    this.url = this.service.getData(patientadmission.FetchPatientEMRIP, {
      RegCode: this.selectedPatientForAdmission.UHID,
      UserId: this.doctorDetails[0]?.UserId ?? this.service.param.UserID,
      WorkStationID: Object.keys(this.facilitySessionId).length ?? this.service.param.WorkStationID,
      HospitalID: this.hospitalID ?? this.service.param.HospitalID,
    });
        ...
        if (response.Code == 200) {
          if (response.FetchPatientDependCLists.length > 1) {
            ...
            if (response.FetchPatientDataCCList.length > 0) {
              ...
              if (this.isFromAdmissionWorklist) {
                this.selectedPatientPatientID = this.akuWrklstPatient.PatientID;
                this.selectedPatientIPID = this.akuWrklstPatient.AdmissionID;
              }
              else {
              ...
              this.AgeUoMID = response.FetchPatientDataCCList[0].AgeUoMID;
  ...
  fetchPatientFromReg() {
    this.url = this.service.getData(patientadmission.FetchPatientFromRegH, {
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          ...
          if (response.FetchRegistrationCompanyPatientDataList?.length > 0) {
            ...
            this.fetchCompanyBedTypes(response.FetchRegistrationCompanyPatientDataList[0].PayerID, response.FetchRegistrationCompanyPatientDataList[0].GradeID);
          }
          else {
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          ...
          if (response.FetchAdmissionWardNBedsList.length > 0) {
            this.FetchAdmissionWardNBedsList = response.FetchAdmissionWardNBedsList;
            this.patientAdmitted = true;
            this.requestedBedTypeEnable = false;
            this.showCancelAdmission = true;
          }
  ...
  fetchCompanyBedTypes(companyid: string, gradeid: string) {
    this.url = this.service.getData(patientadmission.FetchCompanyBedTypes, {
      companyId: companyid == "" ? "0" : companyid,
      gradeId: gradeid == "" ? "0" : gradeid,
      specialisationId: -1,
      workstationId: Object.keys(this.facilitySessionId).length ?? this.service.param.WorkStationID,
    ...
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
          this.eligibleBedTypeList = response.FetchCompanyBedTypesDataList;
          this.allotedBedTypesList = response.FetchCompanyBedTypesDataList;
          if (this.eligibleBedTypeList.length === 0) {
            this.fetchEligibleBedTypes(companyid, gradeid);
          }
          if (this.selectedPatientForAdmission.PatientType == 'EMR') {
            this.loadBedTypesAndWard(this.selectedPatientForAdmission.WardID);
          }
  ...
  selectIsNewBorn() {
    if (this.isNewBorn) {
      ...
      if (this.fromBedsBoard) {
        const motherDetails = JSON.parse(sessionStorage.getItem("InPatientDetails") ?? '{}');
        if (motherDetails) {
          this.motherSelectionForm.patchValue({
            MotherSSN: motherDetails.SSN
          });
          this.fetchMotherDetails(motherDetails.SSN);
        }
      }
      this.isNewBorn = true;
  ...
  loadWardsForSelectedBedType(eligibleBedType: string, wardid: string, bedid: string) {
    ...
    this.url = this.service.getData(patientadmission.FetchAdmissionWards, {
      ...
      GenderID: this.selectedPatientForAdmission.GenderID === undefined ? this.patientRegDetails[0].GenderID : this.selectedPatientForAdmission.GenderID,
      UserID: this.doctorDetails[0]?.UserId ?? this.service.param.UserID,
      WorkStationID: Object.keys(this.facilitySessionId).length ?? this.service.param.WorkStationID,
      HospitalID: this.hospitalID
    });
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200 && response.FetchAdmissionWardsDataList.length > 0) {
          this.wardsList = response.FetchAdmissionWardsDataList;
          if (wardid != "0") {
            setTimeout(() => {
              this.patientAdmissionForm.patchValue({
                WardID: wardid
  ...
  getAgeInYears(dob: string | Date): number {
  ...
  openBiometricModal() {
    ...
    const fingerPrintForAdmissionEnable = this.doctorDetails[0]?.FINGERPRINTENABLEADMISSION;

    const patRegDetails = this.patientRegDetails[0];

    const age = this.getAgeInYears(patRegDetails?.DOB)

    if (age >= 14 && !this.isNewBorn && !this.fromBedsBoard && fingerPrintForAdmission.toLowerCase() === 'yes' && fingerPrintForAdmissionEnable.toLowerCase() === 'yes') {
      this.biometricService.verifyPatient(this.selectedPatientForAdmission, 'Patient Admission')
        .subscribe({
          next: (verified) => {
            if (verified) {
          ...
          error: (error) => {
            ...
            this.errorMessages = [];
            this.errorMessages.push("Biometric verification failed. Please try again.");
            $("#patientAdmissionValidationMsgs").modal('show');
          }
        });
    }
  ...
  savePatientAdmission() {
    ...
    var agreementId = this.patientRegCompanyDetails?.AgreemenID;
    ...
    //Get OPAdmissionID from Adm req from Doctor or from Surgery req if any sur req raised. Otherwise 0
    var OPAdmissionID = "0";
    if (this.selectedPatientForAdmission.PatientID != undefined && this.selectedPatientForAdmission.AdmissionID != undefined) {
      OPAdmissionID = this.selectedPatientForAdmission.AdmissionID;
    }
    else if (this.patientPrevSurgeryReqs.length > 0) {
      OPAdmissionID = this.patientPrevSurgeryReqs[0]?.Admissionid;
    }
    ...
    if (this.patientAdmissionForm.valid && this.patientAdmissionForm.get('EligibleBedType')?.value != '0' && this.patientAdmissionForm.get('AllocBedTypID')?.value != '0'
      ...
      let payload = {
        ...
        "MotherPatientID": this.isNewBorn ? this.selectedMotherDetails?.PatientID : 0,
        "PatientType": 2,
      ...
      this.us.post(patientadmission.SavePatientAdmission, payload).subscribe((response) => {
        if (response.Code == 200) {
          ...
          if (this.selectedPatientForAdmission.length != undefined || this.selectedPatientForAdmission.PatientID != undefined) {
            ...
            // }
            this.patientPrevSurgeryReqs.forEach((element: any) => {
              this.mergeOpToIP(element.SurgeryRequestID, response.AdmissionID);
            });
          }
  ...
  checkForSurgeryRequest() {
    this.url = this.service.getData(patientadmission.FetchPatientLastestOPVisitSurgeryRequest, {
      PatientID: this.selectedPatientPatientID,
    ...
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
          this.patientPrevSurgeryReqs = response.FetchPatientLastestOPVisitSurgeryRequestDataList;
        }
      },
        (err) => {
        })
  ...
  navigateBackToAdmissionRequests() {
    ...
    if (this.isFromAdmissionWorklist) {
      ...
      this.router.navigate(['/admission/admission-assignment-worklist']);
    }
  ...
  onMotherSssnEnter(event: any) {
    ...
    this.fetchMotherDetails(ssn);
  }

  fetchMotherDetails(ssn: any) {
  ...
  FetchPatientAdultBandPrint() {
    ...
    var adultbppayload = {
      ...
      "UserID": this.doctorDetails[0]?.UserId ?? this.service.param.UserID,
      "WorkStationID": this.facility.FacilityID == undefined ? this.facility : this.facility.FacilityID,//Object.keys(this.facilitySessionId).length ?? this.service.param.WorkStationID,
      "Hospitalid": this.hospitalID,
      "JustificationID": 0,
  ...
  PatientPrintCard() {
    this.url = this.service.getData(patientadmission.FetchRegistrationCard, {
      PatientID: this.selectedPatientPatientID,
      UserID: this.doctorDetails[0]?.UserId ?? this.service.param.UserID,
      WorkStationID: Object.keys(this.facilitySessionId).length ?? this.service.param.WorkStationID,
      Hospitalid: this.hospitalID
  ...
  onSpecItemSelected(event: any) {
    ...
    var IsAdult = Number(this.age) >= 14 && Number(this.AgeUoMID) === 1 ? true : false;
    if (IsAdult && item.IsPediatric === '1') {
      ...
      this.errorMessages = [];
      $("#patientAdmissionValidationMsgs").modal('hide');
    }
    this.patientAdmissionForm.patchValue({
  ...
  openCancelAdmissionRemarks() {
    if (this.FetchAdmissionWardNBedsList.length > 0 && this.FetchAdmissionWardNBedsList[0].Status !== '1' && this.FetchAdmissionWardNBedsList[0].Status !== '2') {
      this.errorMsg = "Cannot cancel Admission as patient is admitted in ward.";
      $("#errorMsg").modal('show');
      return;
    }
    $("#cancelAdmissionRemarks").modal('show');
  }
  ...
  cancelAdmission() {
    ...
    let payload = {
      "AdmissionID": this.patientAdmissionDetails.IPID,
      "PatientType": "2",
      "BedID": this.FetchAdmissionWardNBedsList[0].BedID,
      "CancelReason": $("#remarks_text").val(),
      "UserID": this.doctorDetails[0].UserId,
      "WorkStation": Object.keys(this.facilitySessionId).length ?? this.service.param.WorkStationID,
      "HospitalID": this.hospitalID,
    }
    $("#cancelAdmissionRemarks").modal('hide');
    const modalRef = this.modalSvc.open(ValidateEmployeeComponent, {
      backdrop: 'static'
    });
    modalRef.componentInstance.dataChanged.subscribe((data: any) => {
      if (data.success) {
        this.us.post(patientadmission.CancelPatientAdmissions, payload).subscribe((response) => {
          if (response.Code == 200) {
            this.saveMsg = "Patient Admission Cancelled Successfully.";
            $("#cancelAdmissionRemarks").modal('hide');
            $("#patientAdmissionSaveMsg").modal('show');
            this.showCancelAdmission = false;
            this.showSaveBtn = false;
          }
          else {
            if (response.Status == 'Fail') {
  ...
  viewApprovals() {
    var visitid = this.selectedPatientForAdmission.AdmissionID;
    this.config.FetchApprovalRequestAdv(visitid, this.doctorDetails[0].UserId, '3591', this.hospitalID)
      .subscribe((response: any) => {
        ...
        if (response.ApprovalRequestsDataList.length > 0) {
          ...
          this.aprrovalRequestDetails = response.ApprovalRequestDetailsDataList.filter((x: any) => x.ServiceID === '1' || x.ServiceID === '4' || x.ServiceID === '9');
  ...
  showBedAvailability() {
    ...
    const modalRef = this.modalService.openModal(BedsAvailabilityComponent, {
    }, options);
  }

  payerDetails() {
    const url = this.us.getApiUrl(patientadmission.fetchcompanydetails, {
      companyId: this.patientRegCompanyDetails.PayerID,
      hospitalId: this.hospitalID
    });
    this.us.getfullurl(url).subscribe({
      next: (response: any) => {
        if (response.status === 'Success') {
          const modalRef = this.modalSvc.open(PayerDetailsComponent, {
            backdrop: 'static',
            keyboard: false,
            windowClass: 'vte_view_modal'
          });
          modalRef.componentInstance.companyBasicDetails = this.patientRegCompanyDetails;
          modalRef.componentInstance.companyDetails = response?.data || {};
    ...
    if (event.target.value.length >= 3) {
      const searchParam = {
        ...
        inputParams: [
          {
            paramName: '@Param1',
            value: $("#ddlServiceType").val(), // Service Id
            type: 'int',
          },
          {
            paramName: '@Param2',
      ...
      this.us.postfullurl(patientadmission.SearchAllServiceItems, searchParam).subscribe((response) => {
        if (response.status === 'Success') {
          this.allServiceItemsList = response?.data;
        }
      },
        (err) => {
  ...
  onServiceTypeSelected(event: any) {
    if ($("#ddlServiceType").val() == '1') {
      //this.itemPriceList = [];
      this.eligibleBedTypeList.forEach((element: any) => {
        const params = {
          tariffId: -1,
          serviceId: Number($("#ddlServiceType").val()),
          serviceItemId: element.BedTypeID,
          bedTypeId: Number(element.BedTypeID),
          userId: Number(this.doctorDetails[0].UserId),
          workstationId: Object.keys(this.facilitySessionId).length ?? Number(this.service.param.WorkStationID),
          hospitalId: Number(this.hospitalID)
        };
        const url = this.us.getApiUrl(patientadmission.fetchtariffprice, params);
        this.us.getfullurl(url)
          .subscribe((response: any) => {
            if (response.status === 'Success') {
              //this.itemPrice = response?.data.billablePrice
              this.itemPriceList.push({
                "serviceId": "1",
                "serviceType": "Bed Rental",
                "bedType": element.BedType,
                "id": element.BedTypeID,
                "name": element.BedType,
                "price": response?.data.billablePrice
              });
            }
          },
            (err) => {
  ...
  onBedTypeSelected(event: any) {
    this.itemPriceList = [];
    const bedTypeId = event.target.value;
    const name = event.target.options[event.target.options.selectedIndex].text;
    if ($("#ddlServiceType").val() == '1') {
      this.getServiceItemPrice(bedTypeId, name);
    }
  }
  ...
  onServiceItemSelect(item: any) {
    ...
    this.getServiceItemPrice(item.id, item.name);
  }

  getServiceItemPrice(itemId: any, itemName: any) {
    let bedTypeId = -1;
    if ($("#ddlServiceType").val() == '1') {
      bedTypeId = Number(itemId);
    }
    else if ($("#ddlServiceType").val() == '9') {
      bedTypeId = Number($("#EligibleBedType1").val());
    }
    const params = {
      tariffId: -1,
      serviceId: Number($("#ddlServiceType").val()),
    ...
    this.us.getfullurl(url)
      .subscribe((response: any) => {
        if (response.status === 'Success') {
          //this.itemPrice = response?.data.billablePrice
          this.itemPriceList.push({
            "serviceId": $("#ddlServiceType").val(),
            "serviceType": Number($("#ddlServiceType").val()) == 1 ? "Bed Rental" : Number($("#ddlServiceType").val()) == 3 ? 'Investigations' : Number($("#ddlServiceType").val()) == 5 ? 'OP/IP Procedures' : Number($("#ddlServiceType").val()) == 9 ? 'Surgery' : '',
            "bedType": itemName,
            "id": itemId,
            "name": itemName,
            "price": response?.data.billablePrice
  ...
  clearServicePriceInfo() {
    this.itemPriceList = [];
    this.selectedServiceId = "0";
    $("#ddlServiceType").val('');
    $("#EligibleBedType1").val('0');
    $("#invprocSmartSearch").val('');
  }
  ...
  serviceInfoClick() {
    ...
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
          this.serviceInfoeligibleBedTypeList = response.FetchEligibleBedTypesDataList;
        }
      },
        (err) => {
        })
  ...
  printQuotation() {
    if (this.itemPriceList.length == 0 || this.patientRegDetails.length == 0) {
      this.errorMsg = "Please select Patient and add service items to print quotation.";
      $("#errorMsg").modal('show');
      return;
    }
    var payload = {
      "PatientID": this.patientRegDetails[0].PatientID,
      "IPID": this.patientRegDetails[0].AdmissionID,
      "PatientName": this.patientRegDetails[0].FirstName + ' ' + (this.patientRegDetails[0].MiddleName ?? '') + ' ' + this.patientRegDetails[0].LastName + ' ' + (this.patientRegDetails[0].Familyname ?? ''),
      "PatientName2l": this.patientRegDetails[0].FirstName2l + ' ' + (this.patientRegDetails[0].MiddleName2l ?? '') + ' ' + this.patientRegDetails[0].LastName2l + ' ' + (this.patientRegDetails[0].Familyname2l ?? ''),
      "Gender": this.patientRegDetails[0].Gender,
      "DOB": this.patientRegDetails[0].DOB,
      "SSN": this.patientRegDetails[0].SSN,
      "Age": this.patientRegDetails[0].Age + ' ' + this.patientRegDetails[0].AgeType,
      "RegCode": this.patientRegDetails[0].RegCode,
      "Nationality": this.patientRegDetails[0].Nationality,
      "Nationality2l": this.patientRegDetails[0].Nationality2l,
      "MobileNo": this.patientRegDetails[0].MobileNo,
      "Occupation": this.patientRegDetails[0].Occupation,
      "NationalityID": this.patientRegDetails[0].NationalityID,
      "UserID": this.doctorDetails[0]?.UserName + '-' + this.doctorDetails[0]?.EmployeeName,
      "WorkStationID": Object.keys(this.facilitySessionId).length ?? this.service.param.WorkStationID,
      "HospitalID": this.hospitalID,
      "ItemPriceList": JSON.stringify(this.itemPriceList),
    }
    this.us.post(patientadmission.PrintQuotation, payload).subscribe((response) => {
...
export const patientadmission = {
  ...
  FetchPatientFromRegH: 'FetchPatientFromRegH?PatientID=${PatientID}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${Hospitalid}',
  CancelPatientAdmissions: 'CancelPatientAdmissions',
  FetchRegistrationCard: 'FetchRegistrationCard?PatientID=${PatientID}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${Hospitalid}',
  FetchCompanyBedTypes: 'FetchCompanyBedTypes?companyId=${companyId}&gradeId=${gradeId}&specialisationId=${specialisationId}&workstationId=${workstationId}&hospitalId=${hospitalId}',
  FetchPatientLastestOPVisitSurgeryRequest: 'FetchPatientLastestOPVisitSurgeryRequest?PatientID=${PatientID}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  fetchcompanydetails: `${config.rcmapiurl}` + 'OPBilling/fetchcompanydetails?companyId=${companyId}&hospitalId=${hospitalId}',
  SearchAllServiceItems: `${config.rcmapiurl}` + 'CommonData/smartsearchdata',
  fetchtariffprice: `${config.rcmapiurl}` + 'IPBilling/fetchtariffprice?tariffId=${tariffId}&serviceId=${serviceId}&serviceItemId=${serviceItemId}&bedTypeId=${bedTypeId}&userId=${userId}&workstationId=${workstationId}&hospitalId=${hospitalId}',
