...
declare var $: any;
@Component({
    selector: 'app-hemo-dialysis-graft',
    templateUrl: './hemo-dialysis-graft.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class HemoDialysisGraftComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divhbfg') divhbfg!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
    ...
    ngOnInit(): void {
        this.getreoperative("128");
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divhbfg.nativeElement);
        }, 3000);
        this.bindTextboxValue('hemodialysisGraftDate1', moment().format('DD-MMM-YYYY'));
        this.bindTextboxValue('hemodialysisGraftDate2', moment().format('DD-MMM-YYYY'));
        this.bindTextboxValue('hemodialysisGraftDate3', moment().format('DD-MMM-YYYY'));
        this.bindTextboxValue('textbox_SurveillanceMonth', moment().format('MMM-YYYY'));
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divhbfg.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        const tags = this.findHtmlTagIds(this.divhbfg);
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divhbfg.nativeElement.parentNode;
                const nextSibling = this.divhbfg.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divhbfg.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divhbfg.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divhbfg.nativeElement);
                    }
