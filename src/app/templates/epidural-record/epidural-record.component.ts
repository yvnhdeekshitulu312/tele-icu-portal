...
@Component({
  selector: 'app-epidural-record',
  templateUrl: './epidural-record.component.html',
  styleUrls: ['./epidural-record.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class EpiduralRecordComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divepr') divepr!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  //     sameUser = false;
  //   }
  //   this.showElementsData(this.divepr.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser,tem);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            const rowscount = JSON.parse(tem.ClinicalTemplateValues)?.filter((item: any) => item.id.includes('textbox_BloodPressure'));
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divepr);
  ...
  ngAfterViewInit() {
    ...
    if (this.doctordate2) {
      this.doctordate2.nativeElement.id = 'textbox_doctordate2';
    }
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divepr.nativeElement.parentNode;
        const nextSibling = this.divepr.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divepr.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divepr.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divepr.nativeElement);
          }
