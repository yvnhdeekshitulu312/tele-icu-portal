...
declare var $: any;
@Component({
    selector: 'app-radiology-safety-checklist',
    templateUrl: './radiology-safety-checklist.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class RadiologySafetyChecklistComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divrsc') divrsc!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
    ...
    ngOnInit(): void {
        this.getreoperative("136");
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divrsc.nativeElement);
        }, 3000);
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divrsc.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        const tags = this.findHtmlTagIds(this.divrsc);
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divrsc.nativeElement.parentNode;
                const nextSibling = this.divrsc.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divrsc.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divrsc.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divrsc.nativeElement);
                    }
