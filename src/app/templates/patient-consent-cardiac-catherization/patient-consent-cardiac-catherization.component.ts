...
@Component({
  selector: 'app-patient-consent-cardiac-catherization',
  templateUrl: './patient-consent-cardiac-catherization.component.html',
  styleUrls: ['./patient-consent-cardiac-catherization.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class PatientConsentCardiacCatherizationComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divptccc') divptccc!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  ngOnInit(): void {
    this.getreoperative("79");
  }
  ngAfterViewInit() {
    setTimeout(()=>{
      this.showdefault(this.divptccc.nativeElement);
      this.bindTextboxValue('textbox_Name',this.patientData.PatientName);
      this.bindTextboxValue('textbox_NameAr',this.patientData.PatientName);
      this.bindTextboxValue('textbox_Ward',this.patientData?.Ward);
      this.bindTextboxValue('textbox_WardAr',this.patientData?.Ward);      
      this.bindTextboxValue('textbox_Age', this.patientData?.Age);
      this.bindTextboxValue('textbox_AgeAr',this.patientData?.Age);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divptccc.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divptccc);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divptccc.nativeElement.parentNode;
        const nextSibling = this.divptccc.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divptccc.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divptccc.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divptccc.nativeElement);
          }
