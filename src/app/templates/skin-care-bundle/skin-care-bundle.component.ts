...
@Component({
  selector: 'app-skin-care-bundle',
  templateUrl: './skin-care-bundle.component.html',
  styleUrls: ['./skin-care-bundle.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class SkinCareBundleComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divscb') divscb!: ElementRef;
  ...
  //     sameUser = false;
  //   }
  //   this.showElementsData(this.divscb.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divscb.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
            $("#savedModal").modal('hide');
          }
        }
      },
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divscb);
  ...
  ngAfterViewInit() {
    ...
    setTimeout(()=>{
      this.showdefault(this.divscb.nativeElement);
      if (this.divscb) {
        if (this.dischargedate) {
          this.dischargedate.nativeElement.id = 'textbox_dischargedate';
          this.dischargedate.nativeElement.value = this.datepipe.transform(new Date().setDate(new Date().getDate()), "dd-MMM-yyyy")?.toString()
        }
      }
    }, 3000);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divscb.nativeElement.parentNode;
        const nextSibling = this.divscb.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divscb.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divscb.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divscb.nativeElement);
          }
