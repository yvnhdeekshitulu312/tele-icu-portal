...
@Component({
  selector: 'app-critical-care-unit',
  templateUrl: './critical-care-unit.component.html',
  styleUrls: ['./critical-care-unit.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class CriticalCareUnitComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  @ViewChild('divccu') divccu!: ElementRef;
  ...
  @ViewChild('mrpdate', { static: false }) mrpdate!: ElementRef;
  @ViewChild('inchargedate', { static: false }) inchargedate!: ElementRef;
  @ViewChild('thirddate', { static: false }) thirddate!: ElementRef;
  @ViewChild('directordate', { static: false }) directordate!: ElementRef;
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divccu.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divccu);
  ...
  ngAfterViewInit() {
    ...
    if (this.mrpdate) {
      this.mrpdate.nativeElement.id = 'textbox_mrpdate';
    }
    if (this.inchargedate) {
      this.inchargedate.nativeElement.id = 'textbox_inchargedate';
    }
    if (this.thirddate) {
      this.thirddate.nativeElement.id = 'textbox_thirddate';
    }
    if (this.directordate) {
      this.directordate.nativeElement.id = 'textbox_directordate';
    }
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divccu.nativeElement.parentNode;
        const nextSibling = this.divccu.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divccu.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divccu.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divccu.nativeElement);
          }
