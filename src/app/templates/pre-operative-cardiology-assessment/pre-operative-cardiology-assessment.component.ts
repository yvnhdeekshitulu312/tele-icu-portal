...
declare var $: any;
@Component({
  selector: 'app-pre-operative-cardiology-assessment',
  templateUrl: './pre-operative-cardiology-assessment.component.html',
  styleUrls: ['./pre-operative-cardiology-assessment.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class PreOperativeCardiologyAssessmentComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  ...
  searchDoctor(event: any) {
    if (event.target.value.length >= 3) {
      ...
      //   .subscribe((response: any) => {
      //     if (response.Code == 200) {
      //       this.AnethesiadoctorsData = response.FetchHospitalNurseDataaList;
      //     }
      //   },
      //     (err) => {

      //     })
      this.config.FetchSpecialisationDoctorsNP(event.target.value.trim(), 5, this.doctorDetails[0].EmpId, this.hospitalID).subscribe((response: any) => {
        if (response.Code == 200) {
          this.AnethesiadoctorsData = response.FetchDoctorDataList;
        }
      }, error => {
  ...
  onDoctorSelection(item: any, id: string) {
    this.AnethesiadoctorsData = [];
    this.bindTextboxValue(id, item.EmpNo);
  }

  searchCardDoctor(event: any) {
    if (event.target.value.length >= 3) {     
      this.config.FetchSpecialisationDoctorsNP(event.target.value.trim(), 13, this.doctorDetails[0].EmpId, this.hospitalID).subscribe((response: any) => {
        if (response.Code == 200) {
          this.CarddoctorsData = response.FetchDoctorDataList;
        }
      }, error => {
        console.error('Get Data API error:', error);
      });
  ...
  onCarDoctorSelection(item: any, id: string) {
    this.CarddoctorsData = [];
    this.bindTextboxValue(id, item.EmpNo);
  }
