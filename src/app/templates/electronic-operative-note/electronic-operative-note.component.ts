...
@Component({
  selector: 'app-electronic-operative-note',
  templateUrl: './electronic-operative-note.component.html',
  styleUrls: ['./electronic-operative-note.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class ElectronicOperativeNoteComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('diveon') diveon!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  @ViewChild('operativedate', {static: false}) operativedate!: ElementRef;
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.diveon.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.diveon);
  ...
  ngAfterViewInit() {
    ...
    if (this.operativedate) {
      this.operativedate.nativeElement.id = 'textbox_operativedate';
    }
    this.bindTextboxValue('text_Doctor1', this.doctorDetails[0].EmployeeName);
    this.bindTextboxValue('text_DoctorID1', this.doctorDetails[0].EmpNo);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.diveon.nativeElement.parentNode;
        const nextSibling = this.diveon.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.diveon.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.diveon.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.diveon.nativeElement);
          }
