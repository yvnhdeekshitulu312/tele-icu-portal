...
declare var $: any;
@Component({
    selector: 'app-pediatric-intensive-care-unit',
    templateUrl: './pediatric-intensive-care-unit.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class PediatricIntensiveCareUnitComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divpicu') divpicu!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
    ...
    ngOnInit(): void {
        this.getreoperative("121");
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divpicu.nativeElement);
        }, 3000);
        this.bindTextboxValue('textbox_Date', moment().format('DD-MMM-YYYY'));
        const now = new Date();
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divpicu.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        const tags = this.findHtmlTagIds(this.divpicu);
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divpicu.nativeElement.parentNode;
                const nextSibling = this.divpicu.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divpicu.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divpicu.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divpicu.nativeElement);
                    }
