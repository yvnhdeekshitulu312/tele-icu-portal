...
@Component({
  selector: 'app-pre-cardiac-catheterization-checklist',
  templateUrl: './pre-cardiac-catheterization-checklist.component.html',
  styleUrls: ['./pre-cardiac-catheterization-checklist.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class PreCardiacCatheterizationChecklistComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divpccc') divpccc!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  showContent = true;
  listofDoctors: any = [];
  @ViewChild('chkdbynursedate', {static: false}) chkdbynursedate!: ElementRef;
  @ViewChild('mednurdate', {static: false}) mednurdate!: ElementRef;
  @ViewChild('broughtbynurdate', {static: false}) broughtbynurdate!: ElementRef;
  @ViewChild('recbynurdate', {static: false}) recbynurdate!: ElementRef;
  ...
  //     sameUser = false;
  //   }
  //   this.showElementsData(this. divpccc.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser,tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this. divpccc);
  ...
  ngAfterViewInit() {
    // if (this.chkdbynursedate) {
    //   this.chkdbynursedate.nativeElement.id = 'textbox_chkdbynursedate';
    // }
    // if (this.mednurdate) {
    //   this.mednurdate.nativeElement.id = 'textbox_mednurdate';
    // }
    // if (this.broughtbynurdate) {
    //   this.broughtbynurdate.nativeElement.id = 'textbox_broughtbynurdate';
    // }
    // if (this.recbynurdate) {
    //   this.recbynurdate.nativeElement.id = 'textbox_recbynurdate';
    // }

    setTimeout(() => {
      this.showdefault(this.divpccc.nativeElement);
      this.bindTextboxValue('text_PreCatheterization', moment(new Date()).format('DD-MMM-YYYY'));
      this.bindTextboxValue('text_PreCatheterization1', moment(new Date()).format('DD-MMM-YYYY'));
      this.bindTextboxValue('text_PreCatheterization2', moment(new Date()).format('DD-MMM-YYYY'));
      this.bindTextboxValue('text_PreCatheterization3', moment(new Date()).format('DD-MMM-YYYY'));
      this.bindTextboxValue('text_PreCatheterization4', moment(new Date()).format('DD-MMM-YYYY'));
    }, 3000);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divpccc.nativeElement.parentNode;
        const nextSibling = this.divpccc.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divpccc.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divpccc.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divpccc.nativeElement);
          }
    ...
    toggleAsaScoreCheckboxSelection(event: any, groupIds?: string[]) {
    ...
    searchEmployee(event: any) {
      if (event.target.value.length > 2) {
        ...
        this.us.get(this.url)
          .subscribe((response: any) => {
            if (response.Code == 200) {
              this.listofDoctors = response.FetchSSEmployeesDataList;
            }
          },
            (err) => {
            })
