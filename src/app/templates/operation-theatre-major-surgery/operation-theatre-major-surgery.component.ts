...
@Component({
  selector: 'app-operation-theatre-major-surgery',
  templateUrl: './operation-theatre-major-surgery.component.html',
  styleUrls: ['./operation-theatre-major-surgery.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class OperationTheatreMajorSurgeryComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {

  @ViewChild('divotmjs') divotmjs!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  ngOnInit(): void {
    this.getreoperative("104");
  }
  ngAfterViewInit() {
    setTimeout(()=>{
      this.showdefault(this.divotmjs.nativeElement);
    }, 1000);
    if (this.date1) {
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divotmjs.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divotmjs);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divotmjs.nativeElement.parentNode;
        const nextSibling = this.divotmjs.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divotmjs.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divotmjs.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divotmjs.nativeElement);
          }
