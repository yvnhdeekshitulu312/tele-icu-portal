...
@Component({
  selector: 'app-nursing-feeding-chart',
  templateUrl: './nursing-feeding-chart.component.html',
  styleUrls: ['./nursing-feeding-chart.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class NursingFeedingChartComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divnfc') divnfc!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  ngOnInit(): void {
    this.getreoperative("68");
  }

  ngAfterViewInit() {
    setTimeout(()=>{
      this.showdefault(this.divnfc.nativeElement);
    }, 1000);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.showElementsData(this.divnfc.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divnfc);
