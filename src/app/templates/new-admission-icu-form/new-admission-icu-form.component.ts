...
@Component({
  selector: 'app-new-admission-icu-form',
  templateUrl: './new-admission-icu-form.component.html',
  styleUrls: ['./new-admission-icu-form.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class NewAdmissionIcuFormComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divnaif') divnaif!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divnaif.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divnaif);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divnaif.nativeElement.parentNode;
        const nextSibling = this.divnaif.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divnaif.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divnaif.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divnaif.nativeElement);
          }
