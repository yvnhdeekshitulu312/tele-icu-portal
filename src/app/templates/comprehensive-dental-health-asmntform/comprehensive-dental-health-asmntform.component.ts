...
@Component({
  selector: 'app-comprehensive-dental-health-asmntform',
  templateUrl: './comprehensive-dental-health-asmntform.component.html',
  styleUrls: ['./comprehensive-dental-health-asmntform.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class ComprehensiveDentalHealthAsmntformComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divcdha') divcdha!: ElementRef;
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divcdha.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divcdha);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divcdha.nativeElement.parentNode;
        const nextSibling = this.divcdha.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divcdha.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divcdha.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divcdha.nativeElement);
          }
