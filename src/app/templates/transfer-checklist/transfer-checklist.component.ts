...
@Component({
  selector: 'app-transfer-checklist',
  templateUrl: './transfer-checklist.component.html',
  styleUrls: ['./transfer-checklist.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class TransferChecklistComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divtcouo') divtcouo!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  //     sameUser = false;
  //   }
  //   this.showElementsData(this.divtcouo.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser,tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divtcouo);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divtcouo.nativeElement.parentNode;
        const nextSibling = this.divtcouo.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divtcouo.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divtcouo.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divtcouo.nativeElement);
          }
