...
declare var $: any;
@Component({
    selector: 'app-dental-treatment-consent',
    templateUrl: './dental-treatment-consent.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class DentalTreatmentConsentComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divdtcf') divdtcf!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
    ...
    ngOnInit(): void {
        this.getreoperative("125");
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divdtcf.nativeElement);
        }, 3000);
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divdtcf.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        const tags = this.findHtmlTagIds(this.divdtcf);
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divdtcf.nativeElement.parentNode;
                const nextSibling = this.divdtcf.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divdtcf.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divdtcf.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divdtcf.nativeElement);
                    }
