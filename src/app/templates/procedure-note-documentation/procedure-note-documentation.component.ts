...
declare var $: any;
@Component({
    selector: 'app-procedure-note-documentation',
    templateUrl: './procedure-note-documentation.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class ProcedureNoteDocumentationComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divpnd') divpnd!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
  ...
  ProcedureNametextarea: any;
    ...
    ngOnInit(): void {
        this.getreoperative("141");
        this.fetchPatientDataEforms();
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divpnd.nativeElement);
        }, 3000);
    }
    ...
    getSelectedValue(id: any) {
        ...
        if (targetElement) {
            return (targetElement as any).value
        }
        return '';
    }
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divpnd.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        if($("#ProcedureOrderID").val() == '0' && this.selectedView.PatientType == '2') {            
            this.errorMsg = 'Please select Procedure Order';
            $("#errorMsg").modal('show');
            return;
        }

        const tags = this.findHtmlTagIds(this.divpnd);
        ...
        if (tags) {
            ...
            var payload = {
                ...
                "MedicalHistory" : $("#ProcedureName_textarea")[0].innerHTML,
                "AssessmentOrderId" : $("#ProcedureOrderID").val(),
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divpnd.nativeElement.parentNode;
                const nextSibling = this.divpnd.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divpnd.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divpnd.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divpnd.nativeElement);
                    }
    ...
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
          this.patientPlannedProcs = response.FetchPatientProceduresDataEFormsDataList;
        }
      },
        (err) => {
        })
