...
@Component({
  selector: 'app-medical-records-documents-checklist',
  templateUrl: './medical-records-documents-checklist.component.html',
  styleUrls: ['./medical-records-documents-checklist.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class MedicalRecordsDocumentsChecklistComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divnicu') divnicu!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  IsView = false;
  showContent = true;
  @ViewChild('dischargedate1', {static: false}) dischargedate1!: ElementRef;
  @ViewChild('dischargedate2', {static: false}) dischargedate2!: ElementRef;
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divnicu.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divnicu);
  ...
  ngAfterViewInit() {
    if (this.dischargedate1) {
      this.dischargedate1.nativeElement.id = 'textbox_dischargedate1';
    }
    if (this.dischargedate2) {
      this.dischargedate2.nativeElement.id = 'textbox_dischargedate2';
    }
    if (this.checkeddate) {
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divnicu.nativeElement.parentNode;
        const nextSibling = this.divnicu.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divnicu.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divnicu.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divnicu.nativeElement);
          }
