...
@Component({
  selector: 'app-respiratory-therapy',
  templateUrl: './respiratory-therapy.component.html',
  styleUrls: ['./respiratory-therapy.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class RespiratoryTherapyComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  @ViewChild('divresty') divresty!: ElementRef;
  HospitalID: any;
  doctorsData: any[] = [];
  item: any;
  IsPrint = false;
  ...
  ngOnInit(): void {
    this.getreoperative("117");
  }

  ngAfterViewInit() {
    setTimeout(() => {
      this.showdefault(this.divresty.nativeElement);
    }, 3000);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divresty.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divresty, this.requiredFields);
  ...
  templatePrint_Old(name: any) {
    this.print(this.divresty.nativeElement, name);
  }
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divresty.nativeElement.parentNode;
        const nextSibling = this.divresty.nativeElement.nextSibling;

        const space = document.createElement('div');
        space.style.height = '60px';
        header.appendChild(space);

        header.appendChild(this.divresty.nativeElement);

        this.print(header, name);

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divresty.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divresty.nativeElement);
          }
