...
declare var $: any;
@Component({
  selector: 'app-baby-received',
  templateUrl: './baby-received.component.html',
  styleUrls: ['./baby-received.component.scss']
})
export class BabyReceivedComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divbrv') divbrv!: ElementRef;
  ...
  //     sameUser = false;
  //   }
  //   this.showElementsData(this.divbrv.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser,tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divbrv);
  ...
  ngAfterViewInit() {
   if (this.date1) {
    ...
    setTimeout(() => {
      this.showdefault(this.divbrv.nativeElement);  
      const GenderID = `${this.defaultData[1].FetchPatientDataEFormsDataList[0].BabyGenderID}` 
      $("#dropdown_Gender").val(GenderID);  
    }, 4000);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divbrv.nativeElement.parentNode;
        const nextSibling = this.divbrv.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divbrv.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divbrv.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divbrv.nativeElement);
          }
