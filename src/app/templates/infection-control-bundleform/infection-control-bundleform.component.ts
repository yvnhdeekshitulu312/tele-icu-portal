...
@Component({
  selector: 'app-infection-control-bundleform',
  templateUrl: './infection-control-bundleform.component.html',
  styleUrls: ['./infection-control-bundleform.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class InfectionControlBundleformComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  @ViewChild('divicbf') divicbf!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  IsView = false;
  showContent = true;
  @ViewChild('collectedDate', {static: false}) collectedDate!: ElementRef;
  @ViewChild('collected1Date', {static: false}) collected1Date!: ElementRef;
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divicbf.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divicbf);
  ...
  ngAfterViewInit() {
    setTimeout(() => {
      this.showdefault(this.divicbf.nativeElement);
    }, 3000);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divicbf.nativeElement.parentNode;
        const nextSibling = this.divicbf.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divicbf.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divicbf.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divicbf.nativeElement);
          }
