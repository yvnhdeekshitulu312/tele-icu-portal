...
declare var $: any;
@Component({
    selector: 'app-iron-injection-ack',
    templateUrl: './iron-injection-ack.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class IronInjectionAckComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divpaaii') divpaaii!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
    ...
    ngOnInit(): void {
        this.getreoperative("126");
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divpaaii.nativeElement);
        }, 3000);
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divpaaii.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        const tags = this.findHtmlTagIds(this.divpaaii);
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divpaaii.nativeElement.parentNode;
                const nextSibling = this.divpaaii.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divpaaii.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divpaaii.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divpaaii.nativeElement);
                    }
