...
declare var $: any;
@Component({
  selector: 'app-post-conscious-sedation',
  templateUrl: './post-conscious-sedation.component.html',
  styleUrls: ['./post-conscious-sedation.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class PostConsciousSedationComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  @ViewChild('divpcsi') divpcsi!: ElementRef;
  ...
  ngAfterViewInit() {
    ...
    setTimeout(()=>{
      this.showdefault(this.divpcsi.nativeElement);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divpcsi.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divpcsi);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divpcsi.nativeElement.parentNode;
        const nextSibling = this.divpcsi.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divpcsi.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divpcsi.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divpcsi.nativeElement);
          }
