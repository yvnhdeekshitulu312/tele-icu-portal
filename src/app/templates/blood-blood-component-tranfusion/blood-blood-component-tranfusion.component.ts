...
@Component({
  selector: 'app-blood-blood-component-tranfusion',
  templateUrl: './blood-blood-component-tranfusion.component.html',
  styleUrls: ['./blood-blood-component-tranfusion.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class BloodBloodComponentTranfusionComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divbbct') divbbct!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  requiredFields = [
    { id: 'textbox_Representative', message: "Representative", required: true },
    { id: 'textbox_Patient_Rep', message: "Patient or Representative ", required: true },
    { id: 'textbox_name1', message: "Physician Name", required: true },
    //{ id: 'textbox_name2', message: "In case of emergency Physician Name", required: true },
  ];
  ...
  constructor(renderer: Renderer2, el: ElementRef, cdr: ChangeDetectorRef,private service: MedicalAssessmentService, private us: UtilityService) {
    ...
    this.signarureNameCollection = [
      {nameId: 'textbox_Patient_Rep', signatureId: 'Signature1', nameMessage: 'Please enter Patient or Representative ', signatureMessage: 'Please enter Patient or Representative  Signature'},
      {nameId: 'textbox_name1', signatureId: 'Signature5', nameMessage: 'Please enter Physician  Name', signatureMessage: 'Please enter Physician Signature'},
      //{nameId: 'textbox_name2', signatureId: 'Signature7', nameMessage: 'Please enter In case of emergency Physician Name', signatureMessage: 'Please enter emergency Physician Signature'},
      
    ]
   }

  ngOnInit(): void {
    this.getreoperative("73");
  }
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divbbct.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divbbct,this.requiredFields);
  ...
  ngAfterViewInit() {
    ...
    setTimeout(()=>{
      ...
      this.bindTextboxValue("textbox_ID2", this.selectedView.SSN);
      this.bindTextboxValue("textbox_Patient_Rep", this.patientData.PatientName);
      this.bindTextboxValue("textbox_name1", this.doctorDetails[0].UserName);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divbbct.nativeElement.parentNode;
        const nextSibling = this.divbbct.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divbbct.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divbbct.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divbbct.nativeElement);
          }
