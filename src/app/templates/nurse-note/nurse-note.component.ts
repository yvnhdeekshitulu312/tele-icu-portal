...
export class NurseNoteComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divnuno') divnuno!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  IsView = false;
  showContent = true;
  @ViewChild('labor1Date', {static: false}) labor1Date!: ElementRef;
  @ViewChild('labor2Date', {static: false}) labor2Date!: ElementRef;
  @ViewChild('labor3Date', {static: false}) labor3Date!: ElementRef;
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divnuno.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divnuno);
  ...
  ngAfterViewInit() {
    if (this.labor1Date) {
      this.labor1Date.nativeElement.id = 'textbox_labor1Date';
    }
    if (this.labor2Date) {
      this.labor2Date.nativeElement.id = 'textbox_labor2Date';
    }
    if (this.labor3Date) {
      this.labor3Date.nativeElement.id = 'textbox_labor3Date';
    }
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divnuno.nativeElement.parentNode;
        const nextSibling = this.divnuno.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divnuno.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divnuno.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divnuno.nativeElement);
          }
