...
@Component({
  selector: 'app-restraints-monitoring-sheet',
  templateUrl: './restraints-monitoring-sheet.component.html',
  styleUrls: ['./restraints-monitoring-sheet.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class RestraintsMonitoringSheetComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divrms') divrms!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  @ViewChild('renewaltime', {static: false}) renewaltime!: ElementRef;
  @ViewChild('consenttime', {static: false}) consenttime!: ElementRef;
  @ViewChild('patientdate', { static: false }) patientdate!: ElementRef;
  @ViewChildren(MatDatepicker) datepickers1!: QueryList<MatDatepicker<any>>;
  rows: any[] = [];
  ...
  //     sameUser = false;
  //   }
  //   this.showElementsData(this.divrms.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            const rowscount = JSON.parse(tem.ClinicalTemplateValues)?.filter((item: any) => item.id.includes('select_restraint'));
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divrms);
  ...
  ngAfterViewInit() {
    ...
    if (this.renewaltime) {
      this.renewaltime.nativeElement.id = 'textbox_renewaltime';
    }
    if (this.consenttime) {
      this.consenttime.nativeElement.id = 'textbox_consenttime';
    }

    setTimeout(() => {
      this.showdefault(this.divrms.nativeElement);
      this.bindTextboxValue('txt_NurseNameID_1', `${this.doctorData[0].EmpNo}-${this.doctorData[0].EmployeeName}`);
      this.bindTextboxValue('txt_NurseNameID_2', `${this.doctorData[0].EmpNo}-${this.doctorData[0].EmployeeName}`);
      this.bindTextboxValue('txt_NurseNameID_3', `${this.doctorData[0].EmpNo}-${this.doctorData[0].EmployeeName}`);
      this.bindTextboxValue('txt_NurseNameID_4', `${this.doctorData[0].EmpNo}-${this.doctorData[0].EmployeeName}`);
      this.bindTextboxValue('txt_NurseNameID_5', `${this.doctorData[0].EmpNo}-${this.doctorData[0].EmployeeName}`);
    }, 4000);
    ...
    this.addListeners(this.datepickers1);
  }
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divrms.nativeElement.parentNode;
        const nextSibling = this.divrms.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divrms.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divrms.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divrms.nativeElement);
          }
