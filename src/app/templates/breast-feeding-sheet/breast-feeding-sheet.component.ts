...
declare var $: any;
@Component({
  selector: 'app-breast-feeding-sheet',
  templateUrl: './breast-feeding-sheet.component.html',
  styleUrls: ['./breast-feeding-sheet.component.scss']
})
export class BreastFeedingSheetComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divbfs') divbfs!: ElementRef;
  ...
  //     sameUser = false;
  //   }
  //   this.showElementsData(this.divbfs.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divbfs.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
            $("#savedModal").modal('hide');
          }
        }
      },
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divbfs);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divbfs.nativeElement.parentNode;
        const nextSibling = this.divbfs.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divbfs.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divbfs.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divbfs.nativeElement);
          }
