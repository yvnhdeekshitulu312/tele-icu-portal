...
declare var $: any;
@Component({
  selector: 'app-radiology-department-informed-consent',
  templateUrl: './radiology-department-informed-consent.component.html',
  styleUrls: ['./radiology-department-informed-consent.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class RadiologyDepartmentInformedConsentComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  @ViewChild('divrdic') divrdic!: ElementRef;
  ...
  ngAfterViewInit() {
    ...
    setTimeout(() => {
      this.showdefault(this.divrdic.nativeElement);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.showElementsData(this.divrdic.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divrdic);
  ...
  assignYes(id: string, lang: string) {
    ...
    if(id=='toggle_CreatinescreeninglevelneedeeA')
    this.showdefault(this.divrdic.nativeElement);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divrdic.nativeElement.parentNode;
        const nextSibling = this.divrdic.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divrdic.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divrdic.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divrdic.nativeElement);
          }
