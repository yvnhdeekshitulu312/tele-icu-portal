...
@Component({
  selector: 'app-newborn-page',
  templateUrl: './newborn-page.component.html',
  styleUrls: ['./newborn-page.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class NewbornPageComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divnbp') divnbp!: ElementRef;
  ...
  IsView = false;
  showContent = true;
  @ViewChild('peddate', {static: false}) peddate!: ElementRef;
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divnbp.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divnbp);
  ...
  ngAfterViewInit() {
    ...
    setTimeout(()=>{
      this.showdefault(this.divnbp.nativeElement);
      const now = new Date();
      ...
      const idvalue = this.FetcTemplateDefaultDataListM.map((item: any) => {
        const val = item.id_value_pairs.find((pair: any) => pair.id === 'textbox_DOBTime');
        return val ? val.value : null;
      });

      if (idvalue.length > 0) {
        this.timerData.push({ id: 'textbox_generic_time1', value: idvalue[0] })
      }
      else {
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divnbp.nativeElement.parentNode;
        const nextSibling = this.divnbp.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divnbp.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divnbp.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divnbp.nativeElement);
          }
