...
declare var $: any;
@Component({
    selector: 'app-premammography-questionnaire',
    templateUrl: './premammography-questionnaire.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class PremammographyQuestionnaireComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divpmq') divpmq!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
    ...
    ngOnInit(): void {
        this.getreoperative("135");
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divpmq.nativeElement);
        }, 3000);
    }
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divpmq.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        const tags = this.findHtmlTagIds(this.divpmq);
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divpmq.nativeElement.parentNode;
                const nextSibling = this.divpmq.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divpmq.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divpmq.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divpmq.nativeElement);
                    }
