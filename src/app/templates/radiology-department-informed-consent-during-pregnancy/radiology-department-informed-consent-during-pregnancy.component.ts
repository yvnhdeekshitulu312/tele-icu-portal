...
declare var $: any;
@Component({
    selector: 'app-radiology-department-informed-consent-during-pregnancy',
    templateUrl: './radiology-department-informed-consent-during-pregnancy.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class RadiologyDepartmentInformedConsentDuringPregnancyComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divicrdp') divicrdp!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
    ...
    ngOnInit(): void {
        this.getreoperative("134");
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divicrdp.nativeElement);
        }, 3000);
        this.bindTextboxValue('textbox_ExamDate', moment().format('DD-MMM-YYYY'));
        this.bindTextboxValue('textbox_ExamDateA', moment().format('DD-MMM-YYYY'));
        this.bindTextboxValue('textbox_signaturePrintedNameDate', moment().format('DD-MMM-YYYY'));
        this.bindTextboxValue('textbox_signaturePrintedNameDateA', moment().format('DD-MMM-YYYY'));
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divicrdp.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        const tags = this.findHtmlTagIds(this.divicrdp);
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divicrdp.nativeElement.parentNode;
                const nextSibling = this.divicrdp.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divicrdp.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divicrdp.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divicrdp.nativeElement);
                    }
