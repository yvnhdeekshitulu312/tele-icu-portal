...
declare var $: any;
@Component({
  selector: 'app-screening-form-for-mri',
  templateUrl: './screening-form-for-mri.component.html',
  styleUrls: ['./screening-form-for-mri.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class ScreeningFormForMRIComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  @ViewChild('divsfmri') divsfmri!: ElementRef;
  ...
  ngAfterViewInit() {
    ...
    setTimeout(()=>{
      this.showdefault(this.divsfmri.nativeElement);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divsfmri.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divsfmri);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divsfmri.nativeElement.parentNode;
        const nextSibling = this.divsfmri.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divsfmri.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divsfmri.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divsfmri.nativeElement);
          }
