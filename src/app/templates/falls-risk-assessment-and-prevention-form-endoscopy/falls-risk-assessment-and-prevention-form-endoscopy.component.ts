...
@Component({
  selector: 'app-falls-risk-assessment-and-prevention-form-endoscopy',
  templateUrl: './falls-risk-assessment-and-prevention-form-endoscopy.component.html',
  styleUrls: ['./falls-risk-assessment-and-prevention-form-endoscopy.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class FallsRiskAssessmentAndPreventionFormEndoscopyComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  ...
  riskFactorParameterToggle(index: any, item: any) {
    const updateRiskFactorParameter = this.fallRiskFactorParameter.filter(
      (x: any) => x.RiskFactorID === item.RiskFactorID
    );

    updateRiskFactorParameter.forEach((element: any) => {
      if (this.readonly) {
