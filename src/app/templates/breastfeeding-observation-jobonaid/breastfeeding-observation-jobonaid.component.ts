...
@Component({
  selector: 'app-breastfeeding-observation-jobonaid',
  templateUrl: './breastfeeding-observation-jobonaid.component.html',
  styleUrls: ['./breastfeeding-observation-jobonaid.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class BreastfeedingObservationJobonaidComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divbfoj') divbfoj!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  //     sameUser = false;
  //   }
  //   this.showElementsData(this.divbfoj.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser,tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divbfoj);
...
ngAfterViewInit() {
    setTimeout(()=>{
      this.showdefault(this.divbfoj.nativeElement);
    }, 1000);
    ...
    if (this.employeedate) {
      this.employeedate.nativeElement.id = 'textbox_Date1';
      this.employeedate.nativeElement.value = this.datepipe.transform(new Date().setDate(new Date().getDate()), "dd-MMM-yyyy")?.toString()
    }
    this.bindTextboxValue("text_ObserverNameAndSign", this.doctorDetails[0].EmployeeName);
    this.bindTextboxValue("text_ObserverNameAndSignID", this.doctorDetails[0].EmpNo);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divbfoj.nativeElement.parentNode;
        const nextSibling = this.divbfoj.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divbfoj.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divbfoj.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divbfoj.nativeElement);
          }
