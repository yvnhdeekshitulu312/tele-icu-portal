...
declare var $: any;
@Component({
    selector: 'app-psychology-assessment',
    templateUrl: './psychology-assessment.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class PsychologyAssessmentComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divpaf') divpaf!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
    ...
    ngOnInit(): void {
        this.getreoperative("124");
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divpaf.nativeElement);
        }, 3000);
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divpaf.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        const tags = this.findHtmlTagIds(this.divpaf);
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divpaf.nativeElement.parentNode;
                const nextSibling = this.divpaf.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divpaf.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divpaf.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divpaf.nativeElement);
                    }
