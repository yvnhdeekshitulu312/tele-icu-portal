...
declare var $: any;
@Component({
  selector: 'app-patient-id-data-authentication-form',
  templateUrl: './patient-id-data-authentication-form.component.html',
  styleUrls: ['./patient-id-data-authentication-form.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class PatientIDDataAuthenticationFormComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  @ViewChild('divpidaf') divpidaf!: ElementRef;
  ...
  ngOnInit(): void {
    this.getreoperative("98");
  }

  ngAfterViewInit() {
    setTimeout(()=>{
      this.showdefault(this.divpidaf.nativeElement);
    }, 3000);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divpidaf.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divpidaf);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divpidaf.nativeElement.parentNode;
        const nextSibling = this.divpidaf.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divpidaf.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divpidaf.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divpidaf.nativeElement);
          }
