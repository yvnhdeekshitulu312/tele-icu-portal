...
declare var $: any;
@Component({
  selector: 'app-babies-data',
  templateUrl: './babies-data.component.html',
  styleUrls: ['./babies-data.component.scss']
})
export class BabiesDataComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divbdc') divbdc!: ElementRef;
  ...
  //     sameUser = false;
  //   }
  //   this.showElementsData(this.divbdc.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divbdc.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
            $("#savedModal").modal('hide');
          }
        }
      },
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divbdc);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divbdc.nativeElement.parentNode;
        const nextSibling = this.divbdc.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divbdc.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divbdc.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divbdc.nativeElement);
          }
