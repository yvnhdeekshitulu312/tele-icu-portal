...
declare var $: any;
@Component({
    selector: 'app-histopathology-request-form',
    templateUrl: './histopathology-request-form.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class HistopathologyRequestFormComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divhrf') divhrf!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
    ...
    ngOnInit(): void {
        this.getreoperative("137");
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divhrf.nativeElement);
        }, 3000);
        this.bindTextboxValue('textbox_specimendate', moment().format('DD-MMM-YYYY'));
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divhrf.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        const tags = this.findHtmlTagIds(this.divhrf);
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divhrf.nativeElement.parentNode;
                const nextSibling = this.divhrf.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divhrf.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divhrf.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divhrf.nativeElement);
                    }
