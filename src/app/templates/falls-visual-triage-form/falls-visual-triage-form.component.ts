...
@Component({
  selector: 'app-falls-visual-triage-form',
  templateUrl: './falls-visual-triage-form.component.html',
  styleUrls: ['./falls-visual-triage-form.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class FallsVisualTriageFormComponent extends DynamicHtmlComponent implements OnInit {
  @ViewChild('divfvtf') divfvtf!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  @ViewChild('arrivaldate', {static: false}) arrivaldate!: ElementRef;
  @ViewChild('employeedate', {static: false}) employeedate!: ElementRef;
  @ViewChild('employee1date', {static: false}) employee1date!: ElementRef;
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divfvtf.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divfvtf);
  ...
  ngAfterViewInit() {
    setTimeout(()=>{
      this.showdefault(this.divfvtf.nativeElement);
    }, 1000); 

    if (this.arrivaldate) {
      this.arrivaldate.nativeElement.id = 'textbox_arrivaldate';
      this.bindTextboxValue('textbox_arrivaldate', this.datepipe.transform(new Date(), "dd-MMM-yyyy")?.toString())
    }
    if (this.employeedate) {
      this.employeedate.nativeElement.id = 'textbox_employeedate';
      this.bindTextboxValue('textbox_employeedate', this.datepipe.transform(new Date(), "dd-MMM-yyyy")?.toString())
    }

    if(this.employee1date) {
      this.employee1date.nativeElement.id = 'textbox_employee1date';
      this.bindTextboxValue('textbox_employee1date', this.datepipe.transform(new Date(), "dd-MMM-yyyy")?.toString())
    }
    const now = new Date();
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divfvtf.nativeElement.parentNode;
        const nextSibling = this.divfvtf.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divfvtf.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divfvtf.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divfvtf.nativeElement);
          }
