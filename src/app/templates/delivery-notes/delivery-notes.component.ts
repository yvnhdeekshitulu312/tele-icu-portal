...
export class DeliveryNotesComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
    @ViewChild('divdns') divdns!: ElementRef;
    url = "";
    selectedView: any;
    doctorDetails: any;
    ...
    ngOnInit(): void {
        this.getreoperative("127");
    }

    ngAfterViewInit() {
        setTimeout(() => {
            this.showdefault(this.divdns.nativeElement);
        }, 3000);
        const now = new Date();
            ...
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
                        ...
                        this.dataChangesMap = {};
                        this.showElementsData(this.divdns.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
    ...
    save() {
        const tags = this.findHtmlTagIds(this.divdns);
    ...
    templatePrint(name: any, header?: any) {
        if (header) {
            ...
            setTimeout(() => {
                const originalParent = this.divdns.nativeElement.parentNode;
                const nextSibling = this.divdns.nativeElement.nextSibling;

                const space = document.createElement('div');
                space.style.height = '60px';
                header.appendChild(space);

                header.appendChild(this.divdns.nativeElement);

                this.print(header, name);

                setTimeout(() => {
                    if (nextSibling) {
                        originalParent.insertBefore(this.divdns.nativeElement, nextSibling);
                    } else {
                        originalParent.appendChild(this.divdns.nativeElement);
                    }
