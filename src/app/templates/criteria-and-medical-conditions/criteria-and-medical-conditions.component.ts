...
declare var $: any;
@Component({
  selector: 'app-criteria-and-medical-conditions',
  templateUrl: './criteria-and-medical-conditions.component.html',
  styleUrls: ['./criteria-and-medical-conditions.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class CriteriaAndMedicalConditionsComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  @ViewChild('divcmcrp') divcmcrp!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  ngOnInit(): void {
    this.getreoperative("91");
  }

  ngAfterViewInit() {
    setTimeout(()=>{
      this.showdefault(this.divcmcrp.nativeElement);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divcmcrp.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divcmcrp);
