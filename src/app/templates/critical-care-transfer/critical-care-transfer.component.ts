...
@Component({
  selector: 'app-critical-care-transfer',
  templateUrl: './critical-care-transfer.component.html',
  styleUrls: ['./critical-care-transfer.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class CriticalCareTransferComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  @ViewChild('divcct') divcct!: ElementRef;
  url = "";
  selectedView: any;
  doctorDetails: any;
  ...
  ngOnInit(): void {
    this.getreoperative("18");
  }
  ngAfterViewInit() {
    setTimeout(() => {
      this.showdefault(this.divcct.nativeElement);
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNNList.length > 0) {
            ...
            this.dataChangesMap = {};
            this.showElementsData(this.divcct.nativeElement, JSON.parse(tem.ClinicalTemplateValues), sameUser, tem);
  ...
  save() {
    const tags = this.findHtmlTagIds(this.divcct);
  ...
  templatePrint(name: any, header?: any) {
    if (header) {
      ...
      setTimeout(() => {
        const originalParent = this.divcct.nativeElement.parentNode;
        const nextSibling = this.divcct.nativeElement.nextSibling; 

        const space = document.createElement('div');
        space.style.height = '60px'; 
        header.appendChild(space);

        header.appendChild(this.divcct.nativeElement); 

        this.print(header, name); 

        setTimeout(() => {
          if (nextSibling) {
            originalParent.insertBefore(this.divcct.nativeElement, nextSibling);
          } else {
            originalParent.appendChild(this.divcct.nativeElement);
          }
