...
@Component({
  selector: 'app-aku-saftey-checklist',
  templateUrl: './aku-saftey-checklist.component.html',
  styleUrls: ['./aku-saftey-checklist.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class AKUSafteyChecklistComponent extends DynamicHtmlComponent implements OnInit, AfterViewInit {
  @ViewChild('divasct') divasct!: ElementRef;
  ...
  @ViewChild('date2a', { static: false }) date2a!: ElementRef;
  ...
  ngAfterViewInit() {
    ...
    if (this.date2a) {
      this.date2a.nativeElement.value = moment(new Date()).format('DD-MMM-YYYY');
    }
    if (this.date3) {
    ...
    setTimeout(() => {
      this.showdefault(this.divasct.nativeElement);
    ...
    this.config.fetchAdviceDiagnosis(this.selectedView.AdmissionID, this.HospitalID)
      .subscribe((response: any) => {
        if (response.Code == 200 && response.PatientDiagnosisDataList.length > 0) {
          ...
          // });
          diag = response.PatientDiagnosisDataList?.map((item: any) => item.Code + "-" + item.DiseaseName).join('<br/>');
