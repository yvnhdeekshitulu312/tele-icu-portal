...
import { ProcedureappointmentService } from './procedureappointment.service';
...
@Component({
  selector: 'app-procedureappointment',
  templateUrl: './procedureappointment.component.html',
  styleUrls: ['./procedureappointment.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class ProcedureappointmentComponent extends BaseComponent implements OnInit {
  ...
  constructor(private service: ProcedureappointmentService,
    private us: UtilityService, private formbuilder: FormBuilder, public datepipe: DatePipe, private router: Router, private config: SuitConfigService) {
    super()
  ...
  ngOnInit(): void {
    if (this.ward.FacilityName === 'PHYSIOTHERAPY') {
    ...
    this.fromEndoscopy = sessionStorage.getItem("fromEndoscopy") === 'true' ? true : false;
    if (this.fromPhysiotherapy) {
      this.selectedView = JSON.parse(sessionStorage.getItem("PhysiotherapyPatientData") || '{}');
      $("#txtSsn").val(this.selectedView.SSN);
      this.fetchPatientDetails(this.selectedView.SSN, "0", "0");
    }
    if (this.fromEndoscopy) {
      this.selectedView = JSON.parse(sessionStorage.getItem("EndoscopyPatientData") || '{}');
      $("#txtSsn").val(this.selectedView.SSN);
      this.fetchPatientDetails(this.selectedView.SSN, "0", "0");
    }
  ...
  fetchProcedureSearch(event: any) {
    if (event.target.value.length >= 3) {
      this.url = this.service.getData(doctorappointments.fetchGProcedures, { Filter: event.target.value });
      this.us.get(this.url)
        .subscribe((response: any) => {
          if (response.Code == 200) {
  ...
  saveORAppointment(type: string) {
    this.errorMessages = [];
    // if (!this.timesForm.value['FromTime']) {
    //   this.errorMessages.push('Please select From Time');
    // }
    // if (!this.timesForm.value['ToTime']) {
    //   this.errorMessages.push('Please select To Time');
    // }
    // if(this.timesForm.value['FromTime']) {
    //   const fromDate = new Date(`${this.selectedDate.fullDate} ${this.timesForm.value['FromTime']}`);
    //   const currentDate = new Date();
    //   const isBefore = fromDate < currentDate;
    //   if (isBefore) {
    //     this.timesForm.reset();
    //     this.errorMessages.push('Please select future time');
    //   }
    // }
    ...
    let payload = {
      ...
      "ServiceDoctorID": this.selectedDoctor.ServiceDocID,
      "SCHEDULEDATE": this.selectedDate.fullDate,
      "RoomDomainID": this.selectedDoctor.DomainID,
      "DocDomainID": this.fromPhysiotherapy ? 54 : 92,//54,
      "RoomID": this.selectedView.TestID == undefined ? this.form.get('ProcedureID')?.value : this.selectedView.TestID,
      "DOCTORID": this.selectedDoctor.EmpID, //service doc id,
      "SpecialiseID": this.selectedDoctor.SpecialiseId, //service spec id,
      ...
      "Remarks": "PHYSIOTHERAPY Appointment",
      "StatusString": "3",
      "UserID": this.doctorDetails[0]?.UserId ?? this.service.param.UserID,
  ...
  loadVisitInfo(visit: any) {
    ...
    this.form.patchValue({
      ...
      Specialisation: visit.DoctorSpecialty,
    ...
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200 && response.GetAllDoctorsList.length > 0) {
          ...
          this.doctorList.forEach((i: any) => {
            i.Specialization = visit.DoctorSpecialty;
  ...
  rescheduleAppointment(app: any) {
    var reschedulePayload = {
      ...
      "FROMSLOTID": this.selectedTime.FROMSLOTID,
      "TOSLOTID": this.selectedTime.TOSLOTID,
      "Remarks": "Procedure Appointment Portal",
      "SCHEDULESTRING": "",
  ...
  navigateBackWorklist() {
    if (this.fromPhysiotherapy) {
      ...
      sessionStorage.removeItem("PhysiotherapyPatientData");
      this.router.navigate(['/suit/physiotherapyworklist']);
    }
    if (this.fromEndoscopy) {
      sessionStorage.removeItem("fromEndoscopy");
      sessionStorage.removeItem("EndoscopyPatientData");
  ...
  printPhysiotherapySchedule(wrk: any) {
    if (wrk.ScheduleID != '' && wrk.ScheduleID != '0') {
      this.config.printPhysiotherapySchedule(wrk.SCHEDULEID, this.doctorDetails[0].UserId, this.facilityId, this.hospitalID).subscribe((response) => {
        if (response.Status === "Success") {
  ...
  printPatientAllPhysiotherapySchedule() {
    const scheduleids = this.listofUpcomingAppointments.map((item: any) => item.SCHEDULEID).join(',');
    if (scheduleids.length > 0) {
      this.config.printPhysiotherapySchedule(scheduleids, this.doctorDetails[0].UserId, this.facilityId, this.hospitalID).subscribe((response) => {
        if (response.Status === "Success") {
          this.radiologyPdfDetails = response;
          this.trustedUrl = response?.FTPPATH
          $("#reviewAndPayment").modal('show');
  ...
  fetchPastAppointments() {
    ...
    this.url = this.service.getData(doctorappointments.FetchPatientVisitsWithAppointmentsPhy, { FromDate: fromdate, ToDate: todate, PatientID: PatientId, WorkStationID: WorkStationID, HospitalID: this.form.get('HospitalID')?.value ?? this.service.param.HospitalID });
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200 && response.FetchPatientVisitsWithAppointmentsPhyDataList.length > 0) {
          this.latestDoctorVisits = response.FetchPatientVisitsWithAppointmentsPhyDataList;
        } else {
...
export const doctorappointments = {
  ...
  getTimeSlots: 'GettingDateSlotsPhys',
  getClinicTimings: 'GettingDateSlotsPhyN',//'GettingDateSlotsN',
  ...
  FetchPatientVisitsWithAppointmentsPhy: 'FetchPatientVisitsWithAppointmentsPhy?FromDate=${FromDate}&ToDate=${ToDate}&PatientID=${PatientID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}'
};  