...
@Component({
  selector: 'app-neurology-appointment',
  templateUrl: './neurology-appointment.component.html',
  styleUrls: ['./neurology-appointment.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class NeurologyAppointmentComponent extends BaseComponent implements OnInit {
  ...
  @ViewChild('picker') picker!: MatDatepicker<Date>;
    ...
    ngOnInit(): void {
      this.fromNeurology = sessionStorage.getItem("NeuroPatientData") ? true : false;
      if(this.fromNeurology) {
        this.selectedView = JSON.parse(sessionStorage.getItem("NeuroPatientData") || '{}');
        $("#txtSsn").val(this.selectedView.SSN);
        this.fetchPatientDetails(this.selectedView.SSN, "0", "0");
        sessionStorage.removeItem("NeuroPatientData");
      }
    ...
    fetchMedicalEquipments() {
      const url = this.service.getData(doctorappointments.FetchResourceTypeNameScheduleH, { 
          ...
          this.us.get(url)
            .subscribe((response: any) => {
              if (response.Code == 200) {
                this.doctorList = response.FetchResourceTypeNameRadScheduleDataList.filter((x: any) => x?.EquipmentGroup === "NEUROPHYSIOLOGY");
    ...
    saveORAppointment(type: string) {
      ...
      if (this.patientDetails=== undefined) {
        this.errorMessages.push("Select Patient");
      }
    ...
    rescheduleAppointment(app: any) {
      var reschedulePayload = {
        ...
        "FROMSLOTID": this.selectedTime.FROMSLOTID,
        "TOSLOTID": this.selectedTime.TOSLOTID,
        "Remarks": "Neurology Appointment Portal",
        "SCHEDULESTRING": "",
    ...
    onAccept() {
      ...
      sessionStorage.setItem('navigateToNeurologyWorklist', SSN)
