...
export class DoctorappointmentComponent extends BaseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  listOfTimeSlots: any = [];
  listOfTimeSlots1: any = [];
  listOfClinicTimings: any = [];
  listOfClinicTimingsF: any = [];
  selectedDate: any = {};
  selectedTime: any = {};
  ...
  fromBlockedAppointments: boolean = false;
  ...
  prevBillDetails!: FormGroup;
  prevPatientBillDetails: any = [];
  ...
  constructor(private service: DoctorappointmentService, private wardConfig: ConfigService,
    ...
    this.patientRegForm = this.formbuilder.group({
      ...
      Religion: [''],
      AgeUOMID: ['0', Validators.required],
    ...
    this.prevBillDetails = this.formbuilder.group({
      fromDate: prevdate,
      toDate: new Date(),
      serviceId: ['2'],
    });
  ...
  //       var dates1 = this.isDateInRange(date);
  //       if (dates1.length > 0) {
  //         return {
  //           'selected-range-red': true,
  //         };
  ...
  //       var dates = this.isDateInRangeAvail(date);
  //       if (dates.length > 0) {
  //         return {
  //           'selected-range': true,
  //         };
  ...
  // initializePicker(): void {
  //   if (this.picker) {
  //     this.picker.dateClass = (date: Date): MatCalendarCellCssClasses => {
  //       const formattedDate = this.datepipe.transform(date, "dd-MMM-yyyy");

  //       const holiday = this.holidaysList.find((d: any) => d.fullDate === formattedDate);

  //       const appointment = this.listOfAppointments.find((d: any) => d.fullDate === formattedDate);

  //       if (holiday && holiday.cssClass) {
  ...
  ngOnInit(): void {
    ...
    this.fromBlockedAppointments = sessionStorage.getItem("fromBlockedAppointments") === "true" ? true : false;
    ...
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
          ...
          this.listOfDates = [];
          if(!this.fromFutureAppointments && !this.fromBlockedAppointments && !this.fromCoordinatorWorklist) {
            this.patientDetails = [];
          }
          ...
          else if (this.fromFutureAppointments || this.fromCoordinatorWorklist) {
          ...
          else if (this.fromBlockedAppointments) {
            var item = this.doctorList.find((a: any) => a.ServiceDocID == this.dischargefollowups.DoctorID);
            if (item) {
              this.selectedDate.fullDate = moment(new Date(this.dischargefollowups.AppointmentDateTime.split(' ')[0])).format('DD-MMM-YYYY');
              item.HOSPITALID = item.HospitalId;
              item.SPECIALITYID = item.SpecialiseId;
              ...
              if(this.fromBlockedAppointments){
                this.dischargefollowups.SPECIALITYID=this.dischargefollowups.SpecialiseID;
                this.dischargefollowups.SPECIALITY=this.dischargefollowups.SpecialisationName;
                this.dischargefollowups.HOSPITALID=this.dischargefollowups.HospitalID;
                this.dischargefollowups.DOCTORID=this.dischargefollowups.DoctorID;
                this.dischargefollowups.DOCTORNAME=this.dischargefollowups.DoctorName;
                this.dischargefollowups.SCHEDULEID=this.dischargefollowups.ScheduleID;
                this.dischargefollowups.PATIENTNAME=this.dischargefollowups.PatientName;
                this.dischargefollowups.AGE =this.dischargefollowups.Age;
                this.dischargefollowups.AGEUOMID =this.dischargefollowups.AgeUoMID;
              
                setTimeout(() => {
                  this.fetchHospitalLocations();
                }, 100);
               
                this.loadAppointmentForReschedule(this.dischargefollowups);
              }
  ...
  fetchDoctorSearch(event: any) {
    if (event.target.value.length >= 3) {
      ...
      this.us.get(this.url)
        .subscribe((response: any) => {
          if (response.Code == 200) {
            this.listOfItems = response.ReferalDoctorDataList.map((item: any) => ({
              EmpCodeName: item.DoctorName,
  ...
  loadSelectedDoctorAvailability(item: any) {
    ...
    this.url = this.service.getData(doctorappointments.FetchDoctorsWiseNextAvailableBWdatesN, { FromDate: fromdate, ToDate: todate, DoctorID: item.EmpID,SpecialiseID: item.SpecialiseId, HospitalID: this.form.get('HospitalID')?.value ?? this.service.param.HospitalID });
    this.us.get(this.url)
      .subscribe((response: any) => {
    ...
    this.us.post(doctorappointments.getAvailableDatesN, appointmentpayoad)
      .subscribe((response: any) => {
        if (response.Code == 200 && response.DatesList.length > 0) {
  ...
  doctorAvailabilityDateChange() {
    ...
    if (todate != null) {
      ...
      this.loadSelectedDoctorAvailabilityBetweenDates(fromdate, todate, this.selectedDoctor.EmpID,this.selectedDoctor.specialiseID==undefined?this.selectedDoctor.SpecialiseId:this.selectedDoctor.specialiseID, item);
    }
  }

  loadSelectedDoctorAvailabilityBetweenDates(fromdate: any, todate: any, doctorid: any,SpecialiseID:any, item: any) {
    ...
    this.url = this.service.getData(doctorappointments.FetchDoctorsWiseNextAvailableBWdatesN, { FromDate: fromdate, ToDate: todate, DoctorID: doctorid,SpecialiseID: SpecialiseID, HospitalID: this.form.get('HospitalID')?.value ?? this.service.param.HospitalID });
  ...
  addSelectedTime(time: any, type: any) {
    if ((time.ISAVAILABLE == 2 || time.ISAVAILABLE == 4) && time.SSN == null) {
      return;
    }
    if (time.ISAVAILABLE == 3 || time.ISAVAILABLE == 4 || time.ISAVAILABLE == 2|| time.ISAVAILABLE == 5) {
      $("#txtSsn").val(time.SSN);
      this.showSaveBtn = false;
      this.fetchPatientDetails(time.SSN, "0", "0","0");
    }
    else {
  ...
  get7DayRange(index: number): { date: string; label: string }[] {
    const result: { date: string; label: string }[] = [];

    const start = new Date();
    start.setDate(start.getDate() + index * 7); // Offset by 7 * index days
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const label = current.toLocaleDateString('en-US', { weekday: 'long' }); // e.g., "Monday"
      const date = `${current.getDate().toString().padStart(2, '0')}-${current.toLocaleDateString('en-US', monthOptions)}-${current.getFullYear()}`;
  ...
  previousWeek() {
    if (this.currenWeekIndex > 0) {
      this.currenWeekIndex = this.currenWeekIndex - 1;
      this.getWeekData(this.currenWeekIndex);
    }
  }
  ...
  isAvailableDate(date: any) {
    if (this.listOfDates?.length > 0) {
      return this.listOfDates.find((element: any) => element.fullDate === date) ? true : false;
    }
    return false;
  }

  navigateToDate(fullDate: any) {
    this.isWeekView = false;
    const selectedDate = this.listOfDates.find((element: any) => element.fullDate === fullDate);
    this.addSelectedDate(selectedDate);
  }

  getWeekData(index: number = 0) {
    this.timesCollection = [];
    this.currenWeekIndex = index;
    this.weekData = [];
    this.currentWeekDates = this.get7DayRange(index);
    const apiCalls = this.currentWeekDates.map((element: any) => {
      return this.us.post(doctorappointments.getTimeSlotsN, {
        "SpecialityId": this.form.get('SpecialiseID')?.value,
        "HospitalId": this.form.get('HospitalID')?.value ?? this.service.param.HospitalID,
      ...
      next: (responses: any) => {
        ...
        responses.forEach((response: any) => {
          if (response.Status === "Success") {
            weekData.push(response.AppointmentList);
          }
        });
        this.getClinicTimingsF(weekData);
      },
      complete: () => {
  ...
  getClinicTimingsF(weekData: any) {
    var timeslotpayload = {
      "SpecialityId": this.form.get('SpecialiseID')?.value,
      "HospitalId": this.form.get('HospitalID')?.value ?? this.service.param.HospitalID,
      "DoctorId": this.selectedDoctor.EmpID,
      "ScheduleFromDate": this.currentWeekDates[0].date,
      "ScheduleToDate": this.currentWeekDates[this.currentWeekDates.length - 1].date
    }
    this.us.post(doctorappointments.getClinicTimingsF, timeslotpayload)
      .subscribe((response: any) => {
        if (response.AppointmentListN.length > 0) {
          this.timesCollection = this.generateTimeSlots(5, response.AppointmentListN[0].FromDateTime, response.AppointmentListN[0].ToDateTime);
        } else {
          this.timesCollection = this.generateTimeSlots(5, '00:00', '23:59');
        }
        this.weekData = this.currentWeekDates.map((day: any, index: number) => {
            const bookings = weekData[index] || [];
            return {
              day,
              timings: bookings.length > 0 ? `${bookings[0].FROMTIME} - ${bookings[bookings.length - 1].TOTIME}` : '',
              isClickable: this.isAvailableDate(day.date),
              slots: this.timesCollection.map((time: string) => ({
                time,
                booking: null,
                render: true,
                span: 1
              })).map((slot: any, i: any, allSlots: any) => {
                const timeMinutes = this.toMinutes(slot.time);
                const booking = bookings.find((b: any) => timeMinutes >= this.toMinutes(b.FROMTIME) && timeMinutes < this.toMinutes(b.TOTIME));

                if (!booking) return slot;

                // Check if this is the first slot of the booking
                const isStartSlot = timeMinutes === this.toMinutes(booking.FROMTIME);

                if (isStartSlot) {
                  // Count how many slots this booking spans
                  const from = this.toMinutes(booking.FROMTIME);
                  const to = this.toMinutes(booking.TOTIME);
                  const span = allSlots.slice(i).filter((s: any) => {
                    const m = this.toMinutes(s.time);
                    return m >= from && m < to;
                  }).length;
                  ...
                  return {
                    ...slot,
                    booking,
                    render: false, // hide other parts of the booking
                  };
                }
              })
            };
  ...
  getTimeslots(date: any) {
    ...
    this.us.post(doctorappointments.getTimeSlotsN, timeslotpayload)
      .subscribe((response: any) => {
        if (response.Status === "Success") {
          this.getClinicTimings();
          this.listOfTimeSlots = this.listOfTimeSlots1 = response.AppointmentList;
          ...
          this.morningTimeSlots.map((slot: any) => {
            ...
            slot.modToTime = moment(slot.TOTIME, ["HH:mm"]).format("hh:mm A");
          })
  ...
  onMobileSearchPatientSelection(patient: any) {
    ...
    if (this.FetchPatientdetailsLastVisitedDataList.length == 0) {
      this.fetchPatientDetails(patient.SSN === '' ? "0" : patient.SSN, "0", PatientID,"0");
    }
    else {
      this.FetchPatientdetailsLastVisitedDataList.forEach((item: any) => {
        if (item.PatientID === PatientID) {
          this.fetchPatientDetails(patient.SSN, "0", PatientID,"0");
          const now = new Date();
  ...
  onEnterPress(event: any) {
   ...
   onEnterPressUHID(event: any) {
    if (event instanceof KeyboardEvent && event.key === 'Enter') {
      this.fetchPatientDetailsByUHID(event);
    }
  }
  fetchPatientDetailsByUHID(event: any) {
    var inputval = event.target.value;
    var ssn = "0"; var mobileno = "0"; var patientid = "0"; var UHID = "0";
    ...
    this.fetchPatientDetails(ssn, mobileno, patientid,UHID)
  }
  ...
  fetchSelectedPatientDetails(patientid: string) {
    this.fetchPatientDetails("0", "0", patientid,"0");
    $("#divMultiplePatients").modal('hide');
  }

  fetchPatientDetails(ssn: string, mobileno: string, patientId: string, UHID: string,fromSlot: boolean = false) {
    this.showSaveBtn = true;
    ...
    this.url = this.service.fetchPatientDataBySsn(doctorappointments.fetchPatientDataBySsnUHID);
        ...
        if (response.Code == 200) {
          ...
          if (response.FetchPatientDependCLists.length > 1) {
            ...
            if (response.FetchPatientDataCCList.length > 0) {
              ...
              this.selectedPatientSSN = this.patientDetails.SSN;
              //this.AppointmentCount = this.patientDetails.AppointmentCount;
              this.PinblockID = this.patientDetails.PinblockID;
              this.Blockreason = this.patientDetails.Blockreason;
              this.Discription = this.patientDetails.Discription;
              if (this.patientDetails.PinblockID != "") {
                this.showSaveBtn = false;
                this.errorMessages = [];
                this.errorMessages.push(this.patientDetails.Discription);
                $("#doctorAppointmentValidationUB").modal('show');
              }
              this.showVisitOrUpcomingInfo('upcoming');
              if (!fromSlot) {
                this.lastvisited_toggleClass();
  ...
  selectPatient(pat: any) {
    ...
    this.fetchPatientDetails("0", "0", pat.PatientID,"0");
  }
  ...
  saveAppointment() {
    ...
    if (!this.patientDetails || this.patientDetails.length === 0 ) {
    ...
    if (this.multipleSlots) {
      ...
      const allSlots = [...selectedMorningSlots, ...selectedAfternoonSlots, ...selectedEveningSlots];
      const availableSlots = allSlots.filter(slot => slot.ISAVAILABLE.toString() === '1');
      if (allSlots.length !== availableSlots.length) {
        this.errorMessages.push('Please select available slots');
      }
    } else {
      FromSlotId = this.selectedTime.FROMSLOTID;
      ToSlotId = this.selectedTime.TOSLOTID;
      const isAvaialble = this.selectedTime.ISAVAILABLE?.toString();
      if (isAvaialble && isAvaialble !== '1' && isAvaialble !== '9') {
        this.errorMessages.push(`The selected time slot is not available due to ${this.statusMapping[isAvaialble]}. Please choose a different time.`);
      }
    }
    if (this.errorMessages.length === 0) {
      let payload = {
        ...
        "IsVitual": this.selectedTime.ISAVAILABLE == 9 ? 1 : 0,
        "TeleURL": null,
        "StatusString": "3",
  ...
  FetchPatientOPDVisits() {
    ...
    // })
    this.us.post(doctorappointments.FetchPatientOPDVisitsH, payload)
      .subscribe((response: any) => {
        if (response.Code == 200) {
  ...
  fetchUpcomingAppointments() {
    ...
    this.us.post(doctorappointments.AppointmentListNLH, upcomingPayload)
      .subscribe((response: any) => {
        if (response.Code == 200) {
          this.listofUpcomingAppointments = response.AppointmentList;
          this.AppointmentCount = response.AppointmentList.length;
          if(this.fromFutureAppointments || this.fromCoordinatorWorklist) {
            const findapp = this.listofUpcomingAppointments.find((x: any) => x.SCHEDULEID == this.dischargefollowups?.ScheduleID);
            if(findapp) {
              this.loadAppointmentForReschedule(findapp);
            }
          }
        }
      },
    ...
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200 && response.GetAllDoctorsList.length > 0) {
          ...
          this.isMobileSearch = false;
          this.fetchPatientDetails(visit.SSN, "0", "0","0");
  ...
  rescheduleAppointment(app: any) {
    var reschedulePayload = {
      ...
      "MOBILE": this.patientDetails.MobileNo==undefined?app.MobileNo:this.patientDetails.MobileNo,
      "GENDERID": this.patientDetails.GenderID==undefined?app.GenderID:this.patientDetails.GenderID,
      // "DOCTORID": app.DOCTORID,
      // "SpecialiseID": app.SPECIALITYID,
      "DOCTORID": this.selectedDoctor.EmpID,
      "SpecialiseID": this.form.get('SpecialiseID')?.value,
      ...
      "FROMSLOTID": this.selectedTime.FROMSLOTID,
      "TOSLOTID": this.selectedTime.TOSLOTID,
      "Remarks": (this.fromFutureAppointments || this.fromCoordinatorWorklist) ? "Reschedule" :"Reschedule NEW HIS",
      "SCHEDULESTRING": "",
  ...
  closePatientRegPopUp() {
    this.clearPatientRegFields();
    $("#registerPatientModal").modal('hide');
  }

  clearPatientRegFields() {
    this.patientRegForm = this.formbuilder.group({
      TitleID: '',
      Title: '',
      NationalityID: '',
      ...
      OccupationID: '0',
      Occupation: '',
      idExpiryDate: [new Date().setMonth(new Date().getMonth() + 1)],
    });
  }
  ...
  fetchEnglishDate(eve: any) {
    ...
    this.service.getEnglishDate(reqDate).subscribe((response) => {
  ...
  onGenderChange(event: any) {
    ...
    this.patientRegForm
      .get('Gender')
      ?.setValue(event.target.options[event.target.options.selectedIndex].text);
    if (Genderid != '') {
      if (Genderid == 1) {
  ...
  onAgeChange() {
    const age = this.patientRegForm.get('Age')?.value;
    const ageUomid = this.patientRegForm.get('AgeUOMID')?.value;
    ...
    switch (ageUomid) {
      ...
      case "2":
        ...
        if (pastDate.getMonth() < 0) {
          ...
          pastDate.setMonth(11 + pastDate.getMonth());
        }
        break;
      case "3":
        pastDate = new Date(currentDate.getTime());
        pastDate.setDate(currentDate.getDate() - age);
        break;
      default:
        pastDate = currentDate; // Default to current date if ageUomid is not 1, 2, or 3
    }

    this.patientRegForm.patchValue({
      DOB: moment(pastDate).format('yyyy-MM-DD'),
      hijriDOB: this.hijriDateConvert(pastDate),
    });
  }
      ...
      (response) => {
        if (response.status == 'Success') {
          if (response.data.length > 0 && response.data[0].regcode !== '') {
            ...
            this.fetchPatientDetails(inputValue1, '0', '0','0');

          } else {
            this.fetchYakeenDetailsNew(inputValue1, inputValue2, type, inputVal, mobileNo);
          }
        } else {
          this.fetchYakeenDetailsNew(inputValue1, inputValue2, type, inputVal, mobileNo);
        }
  ...
  getPatientDataBYYakeenServiceNew(inputVal: number) {
    var inputValue1 = '';
    var inputValue2 = '';
    var type = '';
    var mobileNo = '';
    ...
    if (inputVal == 1) {
      ...
      inputValue2 = monthNumber + '-' + this.saudiYakeenYearNumber;
      if (
        this.saudiYakeenMonthNumber.toString().trim().length == 0 ||
        this.saudiYakeenYearNumber.toString().trim().length == 0 ||
        this.saudiIdYakeen.toString().length == 0
      ...
      inputValue2 = monthNumber + '-' + this.iqamaYakeenYearNumber;
      ...
      inputValue2 = monthNumber + '-' + this.borderYakeenYearNumber;
  ...
  fetchYakeenDetailsNew(
    ...
    var payload = {
      "ReqFrom": "PatientRegistation",
      "CardType": this.yakeenSearchDisplay,
      "SSN": inputValue1,
      "Hijri": inputValue2,
      "EngDate": '01' + '-' + inputValue2,//"09-03-1968",
      "chkNewBorn": false,
      "NewBornGenderID": "0",
      "NationalityID": "0",
      "GCCNationalityID": "0",
    ...
    this.us.post(doctorappointments.CheckJTTokenValid, payload)
      .subscribe({
        next: (response: any) => {
          if (response.Code == 200) {
            this.isGetCalledForYakeenData = 'No';
            this.yakeenDetailsVerified = true;
            this.yakeenData = response.CheckJTTokenValidDataList[0];
            ...
            let Gender;
            let GenderID;
            if (this.yakeenData?.sexDescAr == '1') {
              this.patientRegForm.get('GenderID')?.setValue(1);
              this.patientRegForm.get('Gender')?.setValue('Male');
              Gender = 'Male';
              GenderID = 1;
            ...
            } else if (this.yakeenData.sexDescAr == '2') {
              this.patientRegForm.get('GenderID')?.setValue(2);
              this.patientRegForm.get('Gender')?.setValue('Female');
              Gender = 'Female';
              GenderID = 2;
            ...
            this.patientRegForm.patchValue({
              Familyname: familyNameYakeen == '' ? ' ' : familyNameYakeen,
              Familyname2L: familyNameYakeen2l,
              MiddleName: middleNameYakeen == '' ? ' ' : middleNameYakeen,
              MiddleName2L: middleNameYakeen2l,
              FirstName: firstNameYakeen == '' ? ' ' :  
              !this.yakeenNewBornBaby ? firstNameYakeen
                : 'Baby Of ' + this.yakeenData.firstNameT,
              FirstName2L: firstNameYakeen2l,
              GrandFatherName: grandFatherNameYakeen== '' ? ' ' : grandFatherNameYakeen,
            ...
            if (this.yakeenData.birthDateG == null ||
              (this.yakeenData.familyName == null && this.yakeenData.familyNameT == null) ||
              (this.yakeenData.fatherName == null && this.yakeenData.fatherNameT == null) ||
              (this.yakeenData.firstName == null && this.yakeenData.firstNameT == null) ||
              (this.yakeenData.grandFatherName == null && this.yakeenData.grandFatherNameT == null) ||
              this.yakeenData.idExpirationDate == null ||
              this.yakeenData.nationalityCode == null || this.yakeenData.sexDescAr == null) {
              this.closeYakeenSearchModal();
      ...
      .subscribe({
        next: (response: any) => {
          if (response.status == 'Success') {
            ...
            if (this.yakeenData.birthDateG?.toString() == '' ||
              (this.yakeenData.familyName?.toString() == '' && this.yakeenData.familyNameT?.toString() == '') ||
              (this.yakeenData.fatherName?.toString() == '' && this.yakeenData.fatherNameT?.toString() == '') ||
              (this.yakeenData.firstName?.toString() == '' && this.yakeenData.firstNameT?.toString() == '') ||
              (this.yakeenData.grandFatherName?.toString() == '' && this.yakeenData.grandFatherNameT?.toString() == '') ||
              this.yakeenData.idExpirationDate?.toString() == '' ||
              this.yakeenData.nationalityCode?.toString() == '' || this.yakeenData.sexDescAr?.toString() == '') {
              this.closeYakeenSearchModal();
              $("#registerPatientModal").modal('show');
              return;
            }
  ...
  onSubmit(type: string) {
    ...
    this.patientRegForm.patchValue({
      idExpiryDate: moment(new Date(new Date().setMonth(new Date().getMonth() + 1))).format('DD-MMM-YYYY'),
      ReligionID: 31,
      MaritalStatusID: 11
    });
    // if (
    ...
    if (
      ...
      this.service
        ...
        .subscribe(
          (response) => {
            if (response.status == 'Success') {
              this.fetchPatientDetails(this.patientRegForm.get('SSN')?.value, '0', '0',"0");
              $('#yakeenServiceModel').modal('hide');
              $('#registerPatientModal').modal('hide');
              this.clearPatientRegFields();
              this.clearYakeenSearchInputs();
  ...
  fetchPatientDetailsFromHIS(ssn: string, patientId: string, mobileno: string,UHID: string, response: any) {
    this.service.fetchPatientDataC = {
      ...this.service.fetchPatientDataC,
      SSN: ssn,
      PatientID: patientId,
  ...
  updateAgeandAgeUomID(dob: any) {
    this.patientRegForm.patchValue({
      hijriDOB: this.hijriDateConvert(dob),
  ...
  saveWaitingListAppointmentPopup() {
    ...
    if (this.errorMessages.length == 0) {
      ...
      if (this.morningTimeSlots.length > 0) {
        this.waitingTimeSlots.push({ Day: "Morning", Time: `${this.morningTimeSlots[0].modTime} - ${this.morningTimeSlots[this.morningTimeSlots.length - 1].modToTime}`, selected: false });
      }
      if (this.afternoonTimeSlots.length > 0) { 
        this.waitingTimeSlots.push({ Day: "Afternoon", Time: `${this.afternoonTimeSlots[0].modTime} - ${this.afternoonTimeSlots[this.afternoonTimeSlots.length - 1].modToTime}`, selected: false });
      }
      if (this.eveningTimeSlots.length > 0) {
        this.waitingTimeSlots.push({ Day: "Evening", Time: `${this.eveningTimeSlots[0].modTime} - ${this.eveningTimeSlots[this.eveningTimeSlots.length - 1].modToTime}`, selected: false });
      }
  ...
  ngAfterViewInit(): void {
    if (sessionStorage.getItem("dischargefollowups")) {
      ...
      this.fetchPatientDetails(this.dischargefollowups.SSN, "0", "0","0");
  ...
  navigateBackToDischargeFollowups() {
    if (this.fromFutureAppointments) {
      ...
      this.router.navigate(['/frontoffice/blockedappointmentworklist']);
    }
    else if(this.fromCoordinatorWorklist) {
    ...
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
          ...
          // this.holidaysList = this.holidaysList.map((holiday, index) => ({
          //   ...holiday,
          //   cssClass: this.cssClasses[index % this.cssClasses.length]
          // }));
  ...
  viewMoreLastVisits() {
    ...
    var PatientId = this.patientDetails.RegCode;
    var WorkStationID = Object.keys(this.facilitySessionId).length > 0 ? this.facilitySessionId : this.service.param.WorkStationID;
    this.url = this.service.getData(doctorappointments.FetchPatientVisitsWithAppointmentsEHR, { FromDate: fromdate, ToDate: todate, PatientID: PatientId, WorkStationID: WorkStationID, HospitalID: this.form.get('HospitalID')?.value ?? this.service.param.HospitalID });
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200 && response.FetchPatientVisitsWithAppointmentsDataList.length > 0) {
  ...
  fetchServices() {
    ...
    this.service.fetchServices(searchParam).subscribe({
      next: (response: any) => {
        if (response.status === 'Success') {
          const serviceItems = response.data ?? [];
          this.allServices = serviceItems;
          this.viewPreviousBills();
          // if (serviceItems.length) {
          //   this.serviceItems = serviceItems.filter(
          //     (data: any) =>
          //       data.patientType == IPatientType.BOTH && data.isVisible == 1 && data.bitBLOCKED == 'False' &&
          //       (this.headerFlag == 'service'
          //         ? data.serviceId !== CONSULTATION_DEAFULT_ID
          //         : true)
          //   );
          // }
        }
  ...
  viewPreviousBills() {
    var serviceId = this.prevBillDetails.get('serviceId')?.value;
    var fromdate = this.datepipe.transform(this.prevBillDetails.get('fromDate')?.value, 'dd-MMM-yyyy');
    var todate = this.datepipe.transform(this.prevBillDetails.get('toDate')?.value, 'dd-MMM-yyyy');

    this.service.getPatientPreviousBillDetails(this.patientDetails.PatientID, fromdate, todate, serviceId, this.hospitalID)
      .subscribe((response: any) => {
        $('#previousBillDetailsModal').modal('show');
        if (response.data.length > 0) {
          this.prevPatientBillDetails = response.data;
        } else {
          this.prevPatientBillDetails = [];
        }
      },
        (err) => { }
      );
  ...
  fetchAppointmentHistory() {
    $('#patientAppointmentsHistory').modal('show');
    var WorkStationID = Object.keys(this.facilitySessionId).length > 0 ? this.facilitySessionId : this.service.param.WorkStationID;
    this.url = this.service.getData(doctorappointments.FetchAllPatientSchedules,
      {
        PatientID: this.patientDetails.PatientID, WorkStationID: WorkStationID, UserID: this.doctorDetails[0].UserId,
        HospitalID: this.form.get('HospitalID')?.value ?? this.service.param.HospitalID
      });
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200 && response.FetchAllPatientSchedulesDataList.length > 0) {
          this.patientAppointmentHistory = response.FetchAllPatientSchedulesDataList;
        }
      },
        (err) => {
        });
  ...
  toggleVideoSlots() {
    console.log('Video slots toggled');
    this.isVideoSlots = !this.isVideoSlots;
    if(this.isVideoSlots) {
      this.listOfTimeSlots = this.listOfTimeSlots1.filter((slot: any) => (slot.ISAVAILABLE == 9||slot.ISAVAILABLE == 3&&slot.IsVitual==true));  
      this.morningTimeSlots = [];
      this.afternoonTimeSlots = [];
      this.eveningTimeSlots = [];
      this.listOfTimeSlots.forEach((value: any, index: any) => {
      ...
      this.listOfTimeSlots = this.listOfTimeSlots1;
      this.morningTimeSlots = [];
      this.afternoonTimeSlots = [];
      this.eveningTimeSlots = [];
      this.listOfTimeSlots.forEach((value: any, index: any) => {
...
export const doctorappointments = {
  ...
  fetchPatientDataBySsnUHID: 'FetchPatientDataCUHID?SSN=${SSN}&MobileNO=${MobileNO}&PatientId=${PatientId}&UHID=${UHID}&UserId=${UserId}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',

  AppointmentListNL: 'AppointmentListNL',
   AppointmentListNLH: 'AppointmentListNLH',
  ...
  getTimeSlots: 'GettingDateSlots',
   getTimeSlotsN: 'GettingDateSlotsPFN',


  getClinicTimings: 'GettingDateSlotsN',
  getClinicTimingsF: 'GettingDateSlotsNNF',
  FetchPatientOPDVisits: 'FetchPatientOPDVisits',
  FetchPatientOPDVisitsH: 'FetchPatientOPDVisitsH',
  ...
  CheckJTTokenValid: 'CheckJTTokenValid',
  FetchAllPatientSchedules: 'FetchAllPatientSchedules?PatientID=${PatientID}&WorkStationID=${WorkStationID}&UserID=${UserID}&HospitalID=${HospitalID}',
  FetchPatientVisitsWithAppointmentsEHR: 'FetchPatientVisitsWithAppointmentsEHR?FromDate=${FromDate}&ToDate=${ToDate}&PatientID=${PatientID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}'
};
