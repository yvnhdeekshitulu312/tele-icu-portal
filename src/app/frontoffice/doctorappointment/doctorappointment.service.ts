...
export class DoctorappointmentService extends BaseComponent {
  ...
  getPatientPreviousBillDetails(patientId: any, fromDate: any, toDate: any, serviceId: any, hospitalId: any) {
    return this.https.get<any>(
      `${config.rcmapiurl}` +
        `OPBilling/getpatientpreviousbilldetails?patientId=${patientId}&fromDate=${fromDate}&toDate=${toDate}&serviceId=${serviceId}&hospitalId=${hospitalId}`
    );
  }
  fetchServices(data: any) {
    return this.https.get<any>(
      `${config.rcmapiurl}` +
        `CommonData/services?workStationId=${data.workStationId}&hospitalId=${data.hospitalId}`
    );
  }
  getEnglishDate(reqData: any) {
    return this.https.post<any>(`${config.rcmapiurl}` + + 'FetchEnglishDate', reqData);
  }
}
