...
@Component({
    selector: 'app-sick-leave-opd-approval',
    templateUrl: './sick-leave-opd-approval.component.html',
    providers: [
        {
            provide: DateAdapter,
...
export class SickLeaveOPDApprovalComponent extends BaseComponent implements OnInit {
    datesForm: any;
    approvalForm: any;
    selectedItem: any;
    url: any;
    ...
    constructor(private us: UtilityService,
        ...
        this.approvalForm = this.formBuilder.group({
            ...
            extendCount: ['', Validators.required]
        });
        ...
        this.us.get(url)
            .subscribe((response: any) => {
                if (response.Code == 200) {
                    this.FetchExtendAndApprovedPatientSickLeaveDataList = response.FetchExtendAndApprovedPatientSickLeaveDataList;
    ...
    onApproveClick(item: any) {
        ...
        this.approvalForm.patchValue({
            extendCount: item.ExtendSickleaveDays,
        });
        $('#sickLeaveApprovalModal').modal('show');
    }

    saveApproval() {
        this.us.post(SickLeaveApproval.UpdateExtendAndApprovedPatientSickLeaveOPD, {
            "SickLeaveID": this.selectedItem.OldSickLeaveID,
            "ExtendSickleaveDays": this.approvalForm.get('extendCount')?.value,
            "OPDManagerApprovedUserID": this.doctorDetails[0]?.UserId,
            "OPDManagerApprovedDate": this.datepipe.transform(new Date(), "dd-MMM-yyyy"),
            "OPDManagerApprovedORRejectedRemarks": this.approvalForm.get('remarks')?.value,
            "UserID": this.doctorDetails[0]?.UserId,
            "WorkStationID": this.facility.FacilityID ?? 0,
            "HospitalID": this.hospitalID
        }).subscribe((response: any) => {
            if (response.Code === 200) {
                this.NewSickleaveID = response.PatientVitalEndoscopyDataaList[0].PreProcedureAssessment;
...
const SickLeaveApproval = {
    ...
    UpdateExtendAndApprovedPatientSickLeaveOPD: 'UpdateExtendAndApprovedPatientSickLeaveOPD',
