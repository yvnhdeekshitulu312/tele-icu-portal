...
@Component({
  selector: 'app-physio-referral-worklist',
  templateUrl: './physio-referral-worklist.component.html',
  styleUrls: ['./physio-referral-worklist.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class PhysioReferralWorklistComponent implements OnInit {
  ...
  openProcedureOrder(item: any) {
    ...
    const modalRef = this.modalService.openModal(ProcedureOrderComponent, {
      data: item,
      readonly: true,
      isFrom: "physiotherapyreferral"
    }, options);
    
  }
  fetchUserFacility() {
    this.config.fetchUserFacility(this.loggedinUserDetails[0].UserId, this.hospitalId)
    .subscribe((response: any) => {
        const facilityid = sessionStorage.getItem("facility");
        const fac = response.FetchUserFacilityDataList.find((x:any) => x.FacilityID == facilityid);
        if (fac) {
