...
import { PatientfolderService } from '../../shared/patientfolder/patientfolder.service';
...
import { links } from '../../shared/patientfolder/patientfolder.utils';
...
@Component({
  selector: 'app-patientfolderml',
  templateUrl: './patientfolderml.component.html',
  styleUrls: ['./patientfolderml.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class PatientfoldermlComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  multiplePatient: any = [];
  errorMsg: any;
  @ViewChild('txtMenu') txtMenu!: ElementRef;
  ctasScore: any;
  ...
  constructor(private service: PatientfolderService, private us: UtilityService, public formBuilder: FormBuilder, private router: Router, private datePipe: DatePipe, private utilityService: UtilityService,
    private modalService: GenericModalBuilder, private changeDetectorRef: ChangeDetectorRef, private sanitizer: DomSanitizer, private presconfig: ConfigService, private suitconfig: SuitConfigService,
    private printService: PrintService, private wardConfig: WardConfig, private emrconfig: EmergencyConfigService, private patientService: PatientAlertsService) {
    super();
    ...
    if (ssnfromothersource) {
      ...
      this.fetchPatientUHIDSSN(this.ssnfromothersource);
    }
    ...
    if (this.SummaryfromCasesheet === 'true') {
    ...
    fromDateFilter.setMonth(toDateFilter.getMonth() - 3);
  ...
  clearSearch() {
    this.txtMenu.nativeElement.value = '';
    this.filteredLinks = cloneDeep(links);
  }
  ...
  ngOnInit() {
    ...
    this.currentdateN = moment(new Date()).format('DD-MMM-YYYY HH:mm');
    ...
    this.patientFolderForm.get('VisitID')?.valueChanges.subscribe(value => {
      if (value) {
        const selectedVisit = this.patientVisits.find((data: any) => data.AdmissionID === value);
        $('#visitInputText').val(selectedVisit?.visitInfo);
      }
    });
  }
  ...
  onMobileEnterPress(event: any) {
    event.preventDefault();
    if (event instanceof KeyboardEvent && event.key === 'Enter') {
      this.onFetchPatient((event.target as any).value);
    }
  }
  onUHIDEnterPress(event: any) {
    event.preventDefault();
    if (event instanceof KeyboardEvent && event.key === 'Enter') {
      this.onFetchPatientUHID((event.target as any).value);
    }
  }
  onFetchPatientUHID(UHID: string) {
    this.emrconfig.FetchEMRPatientDataUHID("0", "0", UHID, this.doctorDetails[0].UserId, this.facility.FacilityID == undefined ? this.facility : this.facility.FacilityID, this.hospitalID)
      .subscribe((response: any) => {
        if (response.FetchEMRPatientDataDataaList.length > 1) {
          ...
          if (response.FetchEMRPatientDataDataaList.length === 1) {
            const patientData = response.FetchEMRPatientDataDataaList[0];
            this.fetchPatientUHIDSSN(patientData.SSN);
          }
          else {
  ...
  onFetchPatient(mobileNo: string) {
    this.emrconfig.FetchEMRPatientData("0", mobileNo, this.doctorDetails[0].UserId, this.facility.FacilityID == undefined ? this.facility : this.facility.FacilityID, this.hospitalID)
      .subscribe((response: any) => {
        if (response.FetchEMRPatientDataDataaList.length > 1) {
          this.multiplePatient = response.FetchEMRPatientDataDataaList;
          this.multiplePatient.forEach((element: any, index: any) => {
  ...
  selectPatient(pat: any) {
    ...
    $("#txtSsn").val(pat.SSN);
    this.fetchPatientUHIDSSN(pat.SSN);
  }

  fetchPatientUHIDSSN(SSN: any) {
    this.noPatientSelected = false;
    this.patientsCollection = [];
    this.url = this.service.getData(patientfolder.FetchPatientUHIDSSN, {
      SSN,
      HospitalID: this.hospitalID ?? this.service.param.HospitalID,
    });
  ...
  isSelectDisable() {
    if (this.patientsCollection.length > 0) {
      return this.patientsCollection.find((element: any) => element.selected) ? false : true;
    }
    return true;
  }
  ...
  fetchPatientVisits() {
    ...
    const url = this.service.getData(patientfolder.FetchPatientVisitsPFN, {
  ...
  fetchPatientVistitInfo() {
    ...
    const url = this.service.getData(patientfolder.FetchPatientVistitInfoN, {
      UHID: this.patientdata.RegCode,
      IsnewVisit: this.patientdata.IsNewVisit,
      Admissionid: this.patientdata.AdmissionID,
      HospitalID: this.patientdata.HospitalID//this.hospitalID
    });
    this.us.get(url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
  ...
  fetchPatientHandOverform(admissionid: any, patientid: any) {
    ...
    this.url = this.service.getData(patientfolder.FetchPatientHandOverformPFN, {
      UHID: this.patientdata.RegCode,
      AdmissionID: this.patientdata.AdmissionID,
      HandOverformID: 0,
      IsnewVisit: this.patientdata.IsNewVisit,
  ...
  fetchPatientSummary(tbl: string, section: string, visitchange: boolean) {
    if (section === 'bloodrequests') {
      ...
      this.url = this.service.getData(patientfolder.FetchPatientSummaryBloodReqPFN, {
        UHID: this.patientdata.RegCode,
        Admissionid: this.patientdata.AdmissionID,
        SearchID: 0,
        IsnewVisit: this.patientdata.IsNewVisit,
    ...
    else if (section === 'mrdfiles') {
      var facid = this.facility.FacilityID == undefined ? this.facility : this.facility.FacilityID;
      if (this.SummaryfromCasesheet === 'true' && facid === undefined) {
        facid = this.facility;
      }
      this.url = this.service.getData(patientfolder.FetchPatientMRDScannedDocsPFN, {
        UHID: this.patientdata.RegCode,
        Admissionid: this.patientdata.AdmissionID,
        SearchID: 0,
        IsnewVisit: this.patientdata.IsNewVisit,
      ...
      // });   
      this.url = this.service.getData(patientfolder.FetchPatientSummaryDiagPFN, {
        UHID: this.patientdata.RegCode,
        Admissionid: this.patientdata.AdmissionID,//this.visitAdmissionId,
        SearchID: 0,
        IsnewVisit: this.patientdata.IsNewVisit,
        WorkStationID: facid,
        HospitalID: this.patientdata.HospitalID//hospid
      });
      ...
      this.url = this.service.getData(patientfolder.FetchPatientAdmissionInvestigationsAndProceduresPFN, {
        UHID: this.patientdata.RegCode,
        Admissionid: 0,//this.patientdata.AdmissionID,//this.admissionID,
        IsnewVisit: this.patientdata.IsNewVisit,
      ...
      this.ActiveTab(this.IsTab, visitchange);
      //this.fetchLabOrderResults();
    }
    else if (section === 'medications') {
      this.FetchPatientMedicationPFN();
    }
      ...
      this.patientdata.IPID = this.patientdata.AdmissionID;
      this.patientdata.PatientId = this.patientdata.PatientID;
      sessionStorage.setItem("InPatientDetails", JSON.stringify(this.patientdata));

    }
    else if (section == '' && this.viewMoreTypeHeader == "Consent Forms") {
      this.showConsentFromTypes('generalconsent');
    }
      ...
      case "2": {
        ...
        response.BloodOrdersDataList.forEach((element: any, index: any) => {
          this.viewMoreData.push({
            ...
            "BloodorderID": element.BloodorderID,
            "RequestTypeName": element.RequestTypeName,
            "SampleCollectedDateTime": element.SampleCollectedDateTime,
            "Components": element.Components,
            "IssueBy": element.IssueBy,
            "IssueByName": element.IssueByName,
  ...
  ActiveTab(tab: any, visitchange: boolean) {
    $('#invprocSmartSearch').val('');
    this.filterDataForm.patchValue({
      searchitem: 0
    });
    this.IsTab = tab;
    this.loadInvRad("0", visitchange);
  }
  ...
  addSelectedReport(dept: any) {
    if (dept.Status == '5' || dept.IsNewVisits == '0') {
  ...
  radioLogyReportPDF() {
    const url = this.service.getData(patientfolder.FetchRadReportGroupPDFN, {
      RegCode: this.patientdata.RegCode ?? this.selectedView.RegCode,
  ...
  fetchLabOrderResults(type: string = '') {
    this.resetPagination();
    this.isLoading = true;
    ...
    this.url = this.service.getData(patientfolder.FetchLabOrderResultsPF,
      {
        fromDate: type === '0' ? '0' : fromdate,
        ToDate: type === '0' ? '0' : todate,
        UHID: this.patientdata.RegCode,
        PatientID: this.patientdata.PatientID,
        AdmissionID: type === '0' ? '0' : this.patientdata.AdmissionID,
        SearchID: searchid,
        HospitalID: type === '0' ? '0' : this.patientdata.HospitalID
      });

    this.us.get(this.url).subscribe((response: any) => {
      this.isLoading = false;
      if (response.Code == 200) {
        this.allData = response.FetchLabOrderResultsDataaList.filter((val: any) => val.IsResult == "4");
        ...
        this.distinctSpecialisations = Array.from(new Set(this.allData.map((item: any) => item.specialisation)));
      }
    });
  }
  ...
  loadNextPage() {
    ...
    const nextPageData = this.allData.slice(startIndex, endIndex);

    if (this.currentPage === 0) {
      this.displayedData = [...nextPageData];
    } else {
      this.displayedData = [...this.displayedData, ...nextPageData];
    }
  ...
  fetchRadOrderResults(type: string = '0') {
    this.data = [];
    this.dataToFilter = [];
  ...
  fetchCardiologyOrderResults(type: string = '0') {
    this.data = [];
    this.dataToFilter = [];
    ...
    //   fromDate: fromdate, ToDate: todate,UHID: this.patientdata.RegCode, PatientID: this.patientdata.PatientID, AdmissionID: this.patientdata.AdmissionID, 
    //   SearchID: searchid, HospitalID: this.patientdata.HospitalID
    // });
  ...
  fetchPFTOrderResults() {
    ...
    this.url = this.service.getData(patientfolder.FetchPFTOrderResultsPFN, {
      fromDate: fromdate, ToDate: todate, UHID: this.patientdata.RegCode, PatientID: this.patientdata.PatientID, AdmissionID: this.patientdata.AdmissionID,
      SearchID: searchid, HospitalID: this.patientdata.HospitalID
    });
    // this.url = this.service.getData(patientfolder.FetchCardOrderResults, {
  ...
  fetchNeurologyOrderResults(type: string = '0') {
    this.data = [];
    this.dataToFilter = [];
  ...
  getItemLabReportPdf() {
    this.url = this.service.getData(patientfolder.FetchLabReportItemlevelGroupPDFN, {
  ...
  getLabReportPdf() {
    this.url = this.service.getData(patientfolder.FetchLabReportGroupPDFN, {
      RegCode: this.patientdata.RegCode,
      IsNewVisit: this.selectedReport.IsNewVisits,
      TestOrderId: this.selectedReport.TestOrderId,
      UserID: this.doctorDetails[0].UserId,
  ...
  getLabDocReportPdf(dept: any) {
    const url = this.service.getData(patientfolder.FetchLabDocumentsPFN, {
      UHID: this.patientdata.RegCode,
      TestOrderID: this.selectedReport.TestOrderId,
      TestOrderItemID: this.selectedReport.TestOrderItemID,
      IsnewVisit: dept.IsNewVisits,
      HospitalID: this.patientdata.HospitalID
    });
    this.us.get(url).subscribe((response: any) => {
      //   if (response.Code == 200) {
      ...
      // }
      if (response.Code == 200) {
        if (response.FetchPatientSummaryMRDFilesDataList[0].byteImage == null) {
          this.trustedUrl = response.FetchPatientSummaryMRDFilesDataList[0]?.Url;
  ...
  FetchPatientFileData() {
    this.url = this.service.getData(patientfolder.FetchPatientSummaryFileData, {
      ...
      PatientID: this.PatientID, AdmissionID: this.admissionID, EpisodeID: this.episodeId,
  ...
  fetchPatientSickLeave() {
    ...
    this.url = this.service.getData(patientfolder.FetchPatientSickLeaveWorkListPFN, { UHID: this.patientdata.RegCode, FromDate: startdate, ToDate: enddate, HospitalID: this.patientdata.HospitalID });
    this.us.get(this.url).subscribe((response: any) => {
      if (response.Code == 200) {
  ...
  fetchPatientMedicalCertificate() {
    ...
    const url = this.service.getData(patientfolder.FetchMedicalCertificateRequestPFIPIDPFN, {
      RegCode: this.patientdata.RegCode,
      AdmissionID: this.patientdata.AdmissionID,
      // FromDate: Fromdate,
      // ToDate: ToDate,
  ...
  FetchPatientMedication() {
    ...
    if (this.visitAdmissionId === '0') {
      ...
      this.con.fetchPatientMedication(this.patientdata.PatientType == '2' ? '2' : '1', this.patientdata.AdmissionID, this.patientdata.PatientID, this.patientdata.HospitalID)
        .subscribe((response: any) => {
          if (response.Code == 200) {
            this.fillMedicationData(response);
  ...
  FetchPatientMedicationPFN() {
    this.viewMoreTypeHeader = "Medications";
    if (this.visitAdmissionId === '0') {
      this.con.fetchPatientMedicationPFN(this.patientdata.RegCode, '0', this.visitAdmissionId, this.patientdata.PatientID, 0, 0, 0, 0)
        .subscribe((response: any) => {
          if (response.Code == 200) {
            this.fillMedicationData(response);
      ...
      this.con.fetchPatientMedicationPFN(this.patientdata.RegCode, this.patientdata.PatientType == '2' ? '2' : '1', this.patientdata.AdmissionID, this.patientdata.PatientID, 0, 0, 0, this.patientdata.HospitalID)
        .subscribe((response: any) => {
          if (response.Code == 200) {
            this.fillMedicationData(response);
  ...
  fetchSSurgeries() {
    if (this.visitAdmissionId === '0') {
      ...
      this.url = this.service.getData(patientfolder.FetchSSurgeriesPFN, {
        UHID: this.patientdata.RegCode,
        AdmissionID: this.patientdata.AdmissionID,
        IsnewVisit: this.patientdata.IsNewVisit,
        HospitalID: this.patientdata.HospitalID
  ...
  fetchMainProgressNote() {
    ...
    this.url = this.service.getData(patientfolder.FetchMainProgressNoteNewPFN,
      {
        UHID: uhid, Admissionid: this.patientdata.AdmissionID, IsnewVisit: this.patientdata.IsNewVisit, UserID: this.doctorDetails[0].UserId,
        WorkStationID: this.facility.FacilityID == undefined ? this.facility : this.facility.FacilityID, HospitalID: this.patientdata.HospitalID
      });
    this.us.get(this.url)
      .subscribe(
        (results) => {
  ...
  fetchNursingInstructions() {
    ...
    this.url = this.service.getData(patientfolder.FetchNursingInstructionsPFN, {
      UHID: this.patientdata.RegCode,
      Admissionid: this.patientdata.AdmissionID,
      IsnewVisit: this.patientdata.IsNewVisit,
      WorkStationID: this.wardID,
  ...
  fetchCarePlan() {
    this.url = this.service.getData(patientfolder.FetchCarePlanPFN, {
      Admissionid: this.patientdata.AdmissionID,//this.admissionID, 
      IsnewVisit: this.patientdata.IsNewVisit,
      HospitalID: this.patientdata.HospitalID
    });
    this.us.get(this.url)
  ...
  fetchSummaryData() {
    ...
    if (todate != null) {
      ...
      this.url = this.service.getData(patientfolder.FetchPatientIntakOutputSavePFN, {
        UHID: this.patientdata.RegCode,
        IPID: this.patientdata.AdmissionID,//this.selectedView.AdmissionID, 
        FromDate: fromdate,
        ToDate: todate,
        IsnewVisit: this.patientdata.IsNewVisit,
        UserId: this.doctorDetails[0].UserId,
      ...
      this.us.get(this.url)
        .subscribe((response: any) => {
          if (response.Code == 200) {
            ...
            //   intakedata = response.FetchPatientIntakOutputSaveListD.filter((x: any) => x.UoMID === ele.UoMID && x.Isintake == 'True');
            //   outputdata = response.FetchPatientIntakOutputSaveListD.filter((x: any) => x.UoMID === ele.UoMID && x.Isintake == 'False');
            ...
            //   //         UserName: intakedata[index] != undefined ? intakedata[index].UserName : "",
  ...
  buildSummaryWithoutOutputGrp(intakedata: any[], outputdata: any[]) {
  ...
  onViewIntakeClick() {
    this.viewData = [];
    var fromdate = this.patientdata.AdmitDate;
    fromdate = moment(fromdate).format('DD-MMM-YYYY');
    var todate = moment(new Date()).format('DD-MMM-YYYY');
    if (todate != null) {
      todate = moment(todate).format('DD-MMM-YYYY');
      this.url = this.service.getData(patientfolder.FetchPatientIntakOutputSave, { IPID: this.patientdata.AdmissionID, FromDate: fromdate, ToDate: todate });
      this.gridData = [];
      this.us.get(this.url)
        .subscribe((response: any) => {
          if (response.Code == 200) {
  ...
  scrollTo(section: string, type: string) {
    this.fetchPatientSummary(type, section, false);
  }
  ...
  FetchPatientadmissionGeneralConsent() {
    this.url = this.service.getData(patientfolder.FetchPatientadmissionGeneralConsent, { AdmissionID: this.patientdata.AdmissionID, HospitalID: this.patientdata.HospitalID });
      ...
      .subscribe((response: any) => {
        if (response.Code == 200) {
          ...
          this.admissionGeneralConsent.forEach((element: any, index: any) => {
            element.Class = "icon-w cursor-pointer";
            element.PName = element.PatientName.split('$')[0]
          })
        }
      },
        (err) => {
  ...
  showConsentFromTypes(type: any) {
    ...
    this.admissionGeneralConsent = []; this.FetchPatAdmMedicalInfConsentDataList = []; this.FetchPatadmAgaintDataList = [];
  ...
  GetVitalsData() {
    ...
    if (this.visitAdmissionId === '0') {
      this.con.FetchPatientVitalsByPatientId_PFUHIDN(this.patientdata.RegCode == undefined ? this.patientdata.RegCode : this.patientdata.RegCode, this.patientdata.AdmissionID, FromDate, ToDate, this.patientdata.HospitalID)
        .subscribe((response: any) => {
          if (response.Code == 200) {
      ...
      if (this.patientdata.PatientType == 1) {
        ...
        this.con.FetchPatientVitalsByPatientIdPatientSummaryNN(this.patientdata.RegCode == undefined ? this.patientdata.RegCode : this.patientdata.RegCode, this.patientdata.AdmissionID, FromDate, ToDate, this.patientdata.HospitalID)
          .subscribe((response: any) => {
            if (response.Code == 200) {
              this.tableVitalsList = response.PatientVitalsDataList;
              const distinctThings = response.PatientVitalsDataList.filter(
  ...
  FetchDrugAdministrationDrugs1(FromDate: any, ToDate: any) {
    this.url = this.service.getData(patientfolder.FetchDrugAdministrationDrugsPFN, {
      FromDate: FromDate,
      ToDate: ToDate,
      IPID: this.patientdata.AdmissionID,
      UHID: this.patientdata.RegCode,
  ...
  getPillClass(sl: any, date: any, drug: any) {
    const value = this.processTime(sl, date, drug);
    let pillName = 'Ppill';
    if (value === 'C') {
      pillName = 'Cpill';
    } else if (value === 'H') {
  ...
  // Component - Update fetchMedicineForChart method
  fetchMedicineForChart(date: any, drugName: any) {
    const result = this.drugDataArray?.filter((data: any) =>
      Date.parse(moment(data.FrequencyDate).format('DD-MMM-YYYY')) === Date.parse(date) &&
      data.Drugname === drugName
    );

    if (result.length > 0) {
      // Transform the time strings into objects with properties
      const transformedTimes = result[0].FrequencyDateTime.map((timeString: string) => {
        const value = this.processTime(timeString, date, drugName);
        let pillName = 'Ppill';
  ...
  // Update processTimeForCompletedStatus to work with the new object structure
  processTimeForCompletedStatus(inputTime: any, inputDate: any, drug: any, Type: number) {
    ...
    // Use inputTime.time since inputTime is now an object
    const timeString = inputTime.time || inputTime;
    const inputDateTime = new Date(parts[2], monthIndex, parts[0],
      timeString.split(':')[0], timeString.split(':')[1]);

    inputDateTime.setHours(inputDateTime.getHours() - 2);

    const filteredData = this.calendarFilteredMedications.filter((item: any) =>
      item.Drugname === drug &&
      item.FrequencyDate === inputDate &&
      item.FrequencyDateTime === timeString
    );

    if (filteredData[0]?.STATUSName === 'Completed') {
      if (Type == 1) ReturnContent = filteredData[0]?.AdminDose;
      if (Type == 2) ReturnContent = filteredData[0]?.AdministeredDoseUOM;
      if (Type == 3) ReturnContent = filteredData[0]?.AdministrationDate;
      if (Type == 4) ReturnContent = filteredData[0]?.AdministeredBy;
      if (Type == 5) ReturnContent = filteredData[0]?.VerifiedUser;
      if (Type == 6) ReturnContent = filteredData[0]?.Remarks;
      if (Type == 7) ReturnContent = filteredData[0]?.DrugAdmReasonSectionName;
      if (Type == 8) ReturnContent = filteredData[0]?.NotAdministeredReason;
      if (Type == 9) ReturnContent = filteredData[0]?.AdministerTimeDeviationReason;
      if (Type == 10) ReturnContent = filteredData[0]?.ManualEntryReason;
  ...
  processTime(inputTime: any, inputDate: any, drugName: any): string {
    let currentDate = new Date();
    ...
    const drug = drugName.split(';')[0];
    const filteredData = this.calendarFilteredMedications.filter((item: any) => item.Drugname === drug && item.FrequencyDate === inputDate && item.FrequencyDateTime === inputTime);
  ...
  openSurgeryNotesModal(sur: any) {
    const options: NgbModalOptions = {
      size: 'xl',
      windowClass: 'vte_view_modal SurgeryNotesModal'
    };
  ...
  private createResultsChart(): void {
    this.testGraphsData = this.testGraphsData.sort((a: any, b: any) => new Date(a.ResultsDate).getTime() - new Date(b.ResultsDate).getTime());
    const dates = this.testGraphsData.map((data: any) => data.ResultsDate);
    const testValues: { [key: string]: number[] } = {};
  ...
  FetchPatientBedSideCareForm() {
    ...
    // });
    this.bedSideitems = [];
    this.url = this.service.getData(patientfolder.FetchPatientBedSideCareFromPFN, {
      UHID: this.patientdata?.RegCode,
      Admissionid: this.patientdata.AdmissionID,
      IsnewVisit: this.patientdata.IsNewVisit,
      WorkStationID: this.wardID,
  ...
  FetchPatientBathingCareForm() {
    this.url = this.service.getData(patientfolder.FetchPatientBathSideCareFromPFN, {
      Admissionid: this.patientdata.AdmissionID,
      FromDate: this.selectedView.AdmitDate,
      ToDate: moment(new Date()).format('DD-MMM-YYYY'),
      IsnewVisit: this.patientdata.IsNewVisit,
  ...
  showVteAssessmentView(type: string) {
    ...
    if (type === 'med') {
      ...
      this.FetchPatientFinalAntenatalVTEFrom();
    }
  }
  //VTE-Medical
  fetchVteRiskAssessment() {
    this.url = this.service.getData(patientfolder.FetchFinalSaveVTERiskAssessmentPFN, {
      UHID: this.patientdata.RegCode,
      AdmissionID: this.patientdata.AdmissionID,
      NursingTaskID: 30,
      RiskFactorSubGroupID: 0,
      tbl: '1',
      IsNewVisit: this.patientdata.IsNewVisit,
      UserID: this.doctorDetails[0].UserId,
      WorkStationID: this.wardID,
  ...
  fetchVteSurgicalRiskAssessment() {
    this.url = this.service.getData(patientfolder.FetchFinalSaveVTESurgicalRiskAssessmentPFN, {
      UHID: this.patientdata.RegCode,
      AdmissionID: this.patientdata.AdmissionID,
      NursingTaskID: 30,
      RiskFactorSubGroupID: 0,
  ...
  FetchPatientFinalObgVTEFrom() {
    ...
    const url = this.service.getData(patientfolder.FetchPatientFinalObgVTEFromPFN, {
      UHID: this.patientdata.RegCode,
      AdmissionID: this.patientdata.AdmissionID,
      PatientObgVTEID: 0,
      IsnewVisit: this.patientdata.IsNewVisit,
  ...
  //VTE-ANTENATAL
  FetchPatientFinalAntenatalVTEFrom() {
    this.url = this.service.getData(patientfolder.FetchPatientAntenatalVTEForm, { PatientVTEAntenatalID: 0, AdmissionID: this.admissionID, UserId: this.doctorDetails[0].UserId, WorkStationID: this.wardID, HospitalID: this.hospitalID });
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
  ...
  view(key: any, value: any) {
    value.AdmissionID = value.Admissionid;
    value.PatientID = value.PatientId;
    sessionStorage.setItem("selectedView", JSON.stringify(this.selectedView));
    const options: NgbModalOptions = {
      size: 'xl',
      windowClass: 'vte_view_modal consentFormModal'
    };
    if (key === 'med') {
      ...
      const modalRef = this.modalService.openModal(VteAntenatalComponent, {
        data: value,
        readonly: true,
        fromEHR: true
      }, options);
  ...
  openBloodRequestModal(item: any) {
    this.selectedView.BloodOrderID = item.BloodorderID;
    sessionStorage.setItem("selectedView", JSON.stringify(this.selectedView));
    const options: NgbModalOptions = {
      size: 'xl',
      windowClass: 'openSurgeryRecordModal',
  ...
  fetchPatientEndoTotalRequest() {
    this.url = this.service.getData(patientfolder.PatientEndoTotalRequestPFN, {
      UHID: this.patientdata.RegCode,
      IPID: this.patientdata.AdmissionID,//this.selectedView.AdmissionID,
      IsnewVisit: this.patientdata.IsNewVisit,
      HospitalID: this.patientdata.HospitalID//this.hospitalID
  ...
  patientPreProcedureAssessmentData(item: any) {
    ...
    this.patientdata.GenderID = this.selectedView.GenderID;
    sessionStorage.setItem("InPatientDetails", JSON.stringify(this.patientdata));
    const options: NgbModalOptions = {
      size: 'xl',
    ...
    const modalRef = this.modalService.openModal(EndoscopeComponent, {
      ...
      readonly: true,
      admissionID: this.patientdata.AdmissionID,
      selectedView: this.patientdata
    }, options);
  }
  fetchDietRequisitionADV() {
    // const url = this.service.getData(patientfolder.FetchDietRequisitionADV, {
    const url = this.service.getData(patientfolder.FetchDietRequisitionADVPFN, {
      UHID: this.patientdata.RegCode,
      AdmissionID: this.patientdata.AdmissionID,
      IsnewVisit: this.patientdata.IsNewVisit,
      UserID: this.doctorDetails[0]?.UserId ?? this.service.param.UserID,
  ...
  FetchBradenScaleView() {
    ...
    this.url = this.service.getData(patientfolder.FetchBradenScaleViewPFN, {
      IPID: this.patientdata.AdmissionID,//this.admissionID,
      FromDate: moment(fromdate).format('DD-MMM-YYYY'),
      ToDate: moment(todate).format('DD-MMM-YYYY'),
      IsnewVisit: this.patientdata.IsNewVisit,
      UserID: this.doctorDetails[0].UserId,
  ...
  FetchFallRiskView() {
    const fromdate = this.bradenScaleForm.get("fromdate")?.value;
    const todate = this.bradenScaleForm.get("todate")?.value;
    this.url = this.service.getData(patientfolder.FetchFallRiskViewPFN, {
      UHID: this.patientdata.RegCode,
      IPID: this.patientdata.AdmissionID,//this.admissionID, 
      FromDate: moment(fromdate).format('DD-MMM-YYYY'),
      ToDate: moment(todate).format('DD-MMM-YYYY'),
  ...
  viewTherapauticBleedingEform(templ: any) {
    this.url = this.service.getData(patientfolder.FetchPatienClinicalTemplateDetailsOT, {
      ClinicalTemplateID: templ.BleedingClinicalTemplateID,
      AdmissionID: this.patientdata.AdmissionID,
      PatientTemplatedetailID: templ.BleedingPatientTemplatedetailID, TBL: 1,
      HospitalID: this.patientdata.HospitalID
    });
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
          if (response.FetchPatienClinicalTemplateDetailsNList.length > 0) {
            const FetchPatienClinicalTemplateDetailsNList = response.FetchPatienClinicalTemplateDetailsNList;
            this.viewEformTemplate(FetchPatienClinicalTemplateDetailsNList[0]);
          }
        }
      },
        (err) => {
  ...
  getreoperative(templateid: any) {
    ...
    this.url = this.service.getData(patientfolder.FetchPatienClinicalTemplateDetailsOT, {
      ClinicalTemplateID: templateid,
      AdmissionID: this.patientdata.AdmissionID,
      PatientTemplatedetailID: 0, TBL: 1,
      HospitalID: this.patientdata.HospitalID
    });
    this.us.get(this.url)
      .subscribe((response: any) => {
  ...
  FetchPatientChiefComplaintAndExaminations() {
    ...
    if (this.patientdata.IsNewVisit == '0') {
      ...
      this.url = this.service.getData(patientfolder.FetchPatientChiefComplaintAndExaminationsPFN,
        {
          UHID: this.patientdata.RegCode,
          Admissionid: this.patientdata.AdmissionID,
          UserId: this.doctorDetails[0].UserId,
  ...
  FetchPatienFolderAssementView() {
    ...
    this.url = this.service.getData(patientfolder.FetchPatienClinicalTemplateDetailsOTPFN,
      {
        AdmissionID: this.patientdata.AdmissionID,//this.patientdata?.AdmissionID,
        UHID: this.patientdata.RegCode,//this.patientdata?.RegCode,
    ...
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
          ...
          this.assessmentsDetails = this.assessmentsDetails.filter((x: any) => (x.ClinicalTemplateID != '70' && x.ClinicalTemplateID != '113'));
        }
        else
  ...
  FetchPainAssessmentOrderItemDetails() {
    ...
    this.url = this.service.getData(patientfolder.FetchPainAssessmentOrderItemDetailsPFN,
      {//FetchPainAssessmentOrderItemDetailsPFN?UHID=${UHID}AdmissionID=${AdmissionID}&FromDate=${FromDate}&ToDate=${ToDate}&tbl=${tbl}&IsNewVisit=${IsNewVisit}
        // &UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
        UHID: this.patientdata.RegCode,
        AdmissionID: this.patientdata.AdmissionID,
        FromDate: moment(fromdate).format('DD-MMM-YYYY'),
        ToDate: moment(todate).format('DD-MMM-YYYY'),
  ...
  FetchPhysiotherapyResults() {
    ...
    this.url = this.service.getData(patientfolder.FetchPatientPhysiotherapyAssessmentOrdersFPFN, {
      //PatientID: this.PatientID,
      UHID: this.patientdata.RegCode,
      AdmissionID: this.patientdata.AdmissionID,
      WorkStationID: this.facility.FacilityID == undefined ? this.facility : this.facility.FacilityID,
  ...
  LoadMedicalCertificateData() {
    var startdate = moment(this.patientdata.AdmitDate).format('YYYY-MM-DD');
    var enddate = moment(new Date()).format('YYYY-MM-DD');
    ...
    const url = this.service.getData(patientfolder.FetchPatientAdmissionMedicalReportWorkListPFN, {
      UHID: this.patientdata.RegCode,
      FromDate: startdate,
      ToDate: enddate,
      AdmissionId: this.patientdata.AdmissionID,
  ...
  FetchPatientDischargeSummaryPrint(medcert: any) {
    if (medcert.IsCNHI == 'True') {
      const commonUrl = this.service.getData(patientfolder.FetchPatientDischargeSummaryPrintCNHI, {
        AdmissionID: medcert.AdmissionID, VirtualDischargeID: medcert.VirtualDischargeID,
        WorkStationID: this.facility.FacilityID == undefined ? this.facility : this.facility.FacilityID, HospitalID: medcert.HospitalID
      });
  ...
  FetchPatientAdmissionMedicalInformedConsent() {
    this.url = this.service.getData(patientfolder.FetchPatientAdmissionMedicalInformedConsent, { AdmissionID: this.patientdata.AdmissionID, HospitalID: this.patientdata.HospitalID });
  ...
  FetchPatientadmissionAgaintConsentForHighRiskOperations() {
    this.url = this.service.getData(patientfolder.FetchPatientadmissionAgaintConsentForHighRiskOperations, { AdmissionID: this.patientdata.AdmissionID, HospitalID: this.patientdata.HospitalID });
  ...
  getSelectedSectionData() {
    ...
    if (this.viewMoreTypeHeader === 'Vitals') {
      ...
      const type = '0';
      this.loadInvRad(type, false);
    }

  }

  loadInvRad(type: string, visitchange: boolean) {
    if (type === '0') {
      if (this.IsTab !== 'L') {
        this.invFilter = false;
      }
      ...
      if (this.IsTab === "L" && !this.invFilter) {
        this.fetchLabOrderResults(type);
      }
      else if (this.IsTab === "R") {
        this.fetchRadOrderResults(type);
      }
      else if (this.IsTab === "C") {
        this.fetchCardiologyOrderResults(type);
      }
      else if (this.IsTab === "N") {
        this.fetchNeurologyOrderResults(type);
      }
      else if (this.IsTab === "P") {
        this.data = [];
        this.fetchPFTOrderResults();
  ...
  onSectionVisitChange(event: any) {
    ...
    this.visitDataLoad(this.admissionID, true);
  }

  searchPatientVisit(event: any) {
    const searchString = event.target.value;
    this.filteredPatientVisits = this.patientVisits.filter((item: any) => item.visitInfo.toLowerCase().indexOf(searchString.toLowerCase()) !== -1);
  }

  onSelectPatientVisit(event: any) {
    this.filteredPatientVisits = this.patientVisits;
    const selectedVisit = this.patientVisits.find((data: any) => data.visitInfo === event.option.value);
    this.admissionID = selectedVisit.AdmissionID;
    this.visitAdmissionId = selectedVisit.AdmissionID;
    this.visitDataLoad(this.admissionID, true);
    this.patientFolderForm.get('VisitID')?.setValue(this.admissionID);
  }

  visitDataLoad(admissionId: any, visitchange: boolean) {
    this.patientdata = this.patientVisits.find((visit: any) => visit.AdmissionID == admissionId);
    ...
    this.us.get(url)
      .subscribe((response: any) => {
        ...
        this.fetchPatientSummary(this.selectedSection.scrollValue, this.selectedSection.scrollName, visitchange);
      },
        (err) => {
  ...
  showNursingDetails() {
    sessionStorage.setItem("selectedView", JSON.stringify(this.selectedView));
    sessionStorage.setItem("InPatientDetails", JSON.stringify(this.selectedView));
    this.viewMoreTypeHeader = "Nursing Checklist";
  }

  trackByFn(index: number, item: any): any {
    return item.TestOrderId || index;
  }

  shouldShowTab(): boolean {
    return this.distinctSpecialisations?.length > 0 && this.IsTab === 'L';
  }

  // getVisitColor(item: any) {
  //   if(item.PatientType == '2') {
  //     return '#2B6ACA';
  //   }else if(item.PatientType == '3') {
  //     return '#24A862';
  //   }
  //   else return '#DA4E4E';
  // }
  getVisitStyles(item: any) {
    if (item.PatientType === '2') {
      return { 'background-color': '#3ac77c', color: 'white' };
    } else if (item.PatientType === '3') {
      return { 'background-color': '#e06262', color: 'white' };
    } else {
      return { 'background-color': '#397ee7', color: 'white' };
    }
  }
  ...
  Savelabresultviews(type: string) {
    ...
    this.presconfig.Savelabresultviews(payload).subscribe(response => {
      if (response.Code == 200) {
        ...
        .subscribe((response: any) => {
          if (response.Code == 200) {
            ...
            item.FetchLabTestComponentsList.forEach((element: any) => {
              ...
              else if (element.ControlType === 'Single Selection') {
                if (element.TextValue != "") {
                  if (element.TextValue.split('#').length > 0) {
                    element.TextValue = element.TextValue.split('#')[0];
                  }
                }
              }
            });
  ...
  openTransfusionEntry(item: any) {
    const options: NgbModalOptions = {
      size: 'xl',
      windowClass: 'vte_view_modal'
    };
    const modalRef = this.modalService.openModal(TransfusionFeedbackComponent, {
  ...
  onVisitFocus() {
    if (!this.filteredPatientVisits?.length) return;

    this.visitInfo.opened.pipe(take(1)).subscribe(() => {
      setTimeout(() => {
        const lastSelectedVisitIndex = this.filteredPatientVisits.findIndex(
          (item: any) => item.AdmissionID === this.admissionID
        );
        if (lastSelectedVisitIndex >= 0 && this.visitAutocompleteTrigger.autocomplete) {
          const panel = this.visitAutocompleteTrigger.autocomplete.panel?.nativeElement;
          const firstOption = panel.querySelector('mat-option');
          const optionHeight = firstOption ? firstOption.getBoundingClientRect().height : 50;
          if (panel) {
            panel.scrollTop = lastSelectedVisitIndex * optionHeight;
          } else {
            panel.scrollTop = 0;
          }
        }
  ...
  FetchDrugAdministrationDrugs(FromDate: any, ToDate: any) {
    ...
    this.us.get(this.url).subscribe(
      (response: any) => {
        if (response.Code == 200) {
          this.calendarMedications = response.FetchDrugAdministrationDrugsDataList;
          this.calendarFilteredMedications = response.FetchDrugAdministrationDrugsDataList;
          ...
          this.drugDataArray = [];
          const groupedData = response.FetchDrugAdministrationDrugsDataList.reduce(
          ...
          this.allDrugs = this.drugDataArray
            .reduce(
              (acc, item) => {
                const name = (item.Drugname || '').trim();
                if (name && !acc.set.has(name)) {
          ...
          this.resetPaginationDrugs();
          this.loadNextBatch();
        }
      },
      (err) => {
        console.error('Error fetching drug administration drugs:', err);
      }
    );
  }

  private resetPaginationDrugs(): void {
    this.currentBatch = 0;
    this.drugsList = [];
    this.hasMoreDataDrugs = true;
    this.isLoadingDrugs = false;
    this.isScrolling = false;
  }

  private loadNextBatch(): void {
    if (this.isLoadingDrugs || !this.hasMoreDataDrugs || this.isScrolling) {
      return;
    }
    ...
    setTimeout(() => {
      ...
      const nextBatch = this.allDrugs.slice(startIndex, endIndex);

      if (nextBatch.length > 0) {
        this.drugsList = [...this.drugsList, ...nextBatch];
        this.currentBatch++;
        this.hasMoreDataDrugs = endIndex < this.allDrugs.length;
      } else {
        this.hasMoreDataDrugs = false;
      }
  ...
  private checkScrollPosition(element: HTMLElement): void {
    ...
    if (position > height - threshold && !this.isLoadingDrugs && this.hasMoreDataDrugs && !this.isScrolling) {
      this.loadNextBatch();
    }
  }

  trackByDrug(index: number, drug: string): string {
    return drug;
  }
  ...
  //   if (position > height - threshold && !this.isLoadingDrugs && this.hasMoreDataDrugs) {
  //     this.loadNextBatch();
  //   }
  // }

  showFollowupData() {
    this.viewMoreTypeHeader = "FollowUp";
    const url = this.us.getApiUrl(patientfolder.FetchFollowUpDetailsPFN, {
      UHID: this.patientdata.RegCode,
      admissionId: this.patientdata.AdmissionID,
      userId: this.doctorDetails[0].UserId,
      WorkStationID: "0",
    ...
    this.us.get(url).subscribe((response: any) => {
      if (response.Code === 200) {
        this.followupDetails = response.FetchFollowUpDetailsPFNDataList;
      }
    });
  }
  ...
  showReferralsData() {
    ...
    const url = this.us.getApiUrl(patientfolder.FetchReferalOrdersPFN, {
      UHID: this.patientdata.RegCode,
      admissionId: this.patientdata.AdmissionID,
      userId: this.doctorDetails[0].UserId,
      WorkStationID: "0",
    ...
    this.us.get(url).subscribe((response: any) => {
      if (response.Code === 200) {
        this.referralDetails = response.FetchReferalOrdersPFNDataList;
      }
    });
  }

  searchOPDrugAdministration() {
    this.viewMoreTypeHeader = "OP Drug Administration";
    this.FetchPatientPrescriptionInjectionSchedulesDataList = [];
    const url = this.us.getApiUrl(patientfolder.FetchPatientPrescriptionInjectionSchedules, {
      SSN: this.selectedView.SSN,
      PrescriptionID: '0',
    ...
    this.us.get(url)
      .subscribe((response: any) => {
        if (response.Code === 200) {
          this.FetchPatientPrescriptionInjectionSchedulesDataList = response.FetchPatientPrescriptionInjectionSchedulesDataList.filter((x: any) => x.AdmissionID == this.patientdata.AdmissionID);
        }
      },
        () => {
        });
  ...
  fetchEyeInformation() {
    this.viewMoreTypeHeader = "Eye Information";
    this.FetchPatientPrescriptionInjectionSchedulesDataList = [];
    const url = this.us.getApiUrl(patientfolder.FetchPatientEyeLensInfo, {
      PatientID: this.patientdata.PatientID,
      AdmissionID: this.patientdata.AdmissionID,
      WorkStationID: this.facility.FacilityID,
      HospitalID: this.hospitalID
      ...
      .subscribe((response: any) => {
        if (response.Code === 200) {
          if (response.FetchPatientEyeLensInfoDataList.length > 0) {
            this.eyeInformationData = response.FetchPatientEyeLensInfoDataList[0];
          }
        }
      },
        () => {
...
export const patientfolder = {
  ...
  FetchNursingInstructionsPFN: 'FetchNursingInstructionsPFN?UHID=${UHID}&Admissionid=${Admissionid}&IsnewVisit=${IsnewVisit}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',

  FetchCarePlan: 'FetchCarePlan?Admissionid=${Admissionid}&HospitalID=${HospitalID}',
  FetchCarePlanPFN: 'FetchCarePlanPFN?Admissionid=${Admissionid}&IsnewVisit=${IsnewVisit}&HospitalID=${HospitalID}',
  ...
  FetchDrugAdministrationDrugsPFN: 'FetchDrugAdministrationDrugsPFN?FromDate=${FromDate}&ToDate=${ToDate}&IPID=${IPID}&UHID=${UHID}&IsnewVisit=${IsnewVisit}&HospitalID=${HospitalID}',
  ...
  FetchMedicalCertificateRequestPFIPIDPFN: 'FetchMedicalCertificateRequestPFIPIDPFN?RegCode=${RegCode}&AdmissionID=${AdmissionID}&IsnewVisit=${IsnewVisit}&HospitalID=${HospitalID}',
  ...
  FetchPatientSummaryBloodReqPFN: 'FetchPatientSummaryBloodReqPFN?UHID=${UHID}&Admissionid=${Admissionid}&SearchID=0&IsnewVisit=${IsnewVisit}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  FetchPatientMRDScannedDocsPFN: 'FetchPatientMRDScannedDocsPFN?UHID=${UHID}&Admissionid=${Admissionid}&SearchID=0&IsnewVisit=${IsnewVisit}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  FetchLabDocumentsPFN: 'FetchLabDocumentsPFN?UHID=${UHID}&TestOrderID=${TestOrderID}&TestOrderItemID=${TestOrderItemID}&IsnewVisit=${IsnewVisit}&HospitalID=${HospitalID}',
  ...
  FetchRadReportGroupPDFN: 'FetchRadReportGroupPDFN?RegCode=${RegCode}&TestOrderItemId=${TestOrderItemId}&TestOrderId=${TestOrderId}&IsnewVisit=${IsnewVisit}&UserID=${UserID}&HospitalID=${HospitalID}',
  ...
  FetchNeurologyOrderResultsPF: 'FetchNeurologyOrderResultsPFN?fromDate=${fromDate}&ToDate=${ToDate}&UHID=${UHID}&PatientID=${PatientID}&AdmissionID=${AdmissionID}&SearchID=${SearchID}&HospitalID=${HospitalID}',
  FetchDentalOrderResultsPF: 'FetchDentalOrderResultsPFN?fromDate=${fromDate}&ToDate=${ToDate}&UHID=${UHID}&PatientID=${PatientID}&AdmissionID=${AdmissionID}&SearchID=${SearchID}&HospitalID=${HospitalID}',
  ...
  FetchPFTOrderResultsPFN: 'FetchPFTOrderResultsPFN?fromDate=${fromDate}&ToDate=${ToDate}&UHID=${UHID}&PatientID=${PatientID}&AdmissionID=${AdmissionID}&SearchID=${SearchID}&HospitalID=${HospitalID}',
  ...
  //etchNeurologyOrderResultsPF: 'FetchNeurologyOrderResultsPF?fromDate=${fromDate}&ToDate=${ToDate}&PatientID=${PatientID}&AdmissionID=${AdmissionID}&SearchID=${SearchID}&HospitalID=${HospitalID}',
  ...
  FetchPatientBathSideCareFromPFN: 'FetchPatientBathSideCareFromPFN?Admissionid=${Admissionid}&FromDate=${FromDate}&ToDate=${ToDate}&IsnewVisit=${IsnewVisit}&HospitalID=${HospitalID}',
  FetchPatientBedSideCareFromPFN: 'FetchPatientBedSideCareFromPFN?UHID=${UHID}&Admissionid=${Admissionid}&IsnewVisit=${IsnewVisit}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  PatientEndoTotalRequestPFN: 'PatientEndoTotalRequestPFN?UHID=${UHID}&IPID=${IPID}&IsnewVisit=${IsnewVisit}&HospitalID=${HospitalID}',
  ...
  FetchBradenScaleViewPFN: 'FetchBradenScaleViewPFN?IPID=${IPID}&FromDate=${FromDate}&ToDate=${ToDate}&IsnewVisit=${IsnewVisit}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  FetchPatientChiefComplaintAndExaminationsPFN: 'FetchPatientChiefComplaintAndExaminationsPFN?UHID=${UHID}&Admissionid=${Admissionid}&UserId=${UserId}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  FetchPatienClinicalTemplateDetailsOTPFN: 'FetchPatienClinicalTemplateDetailsOTPFN?AdmissionID=${AdmissionID}&UHID=${UHID}&ClinicalTemplateID=${ClinicalTemplateID}&PatientTemplatedetailID=${PatientTemplatedetailID}&TBL=${TBL}&IsnewVisit=${IsnewVisit}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  FetchFallRiskViewPFN: 'FetchFallRiskView?UHID=${UHID}&IPID=${IPID}&FromDate=${FromDate}&ToDate=${ToDate}&IsnewVisit=${IsnewVisit}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  FetchPatientIntakOutputSavePFN: 'FetchPatientIntakeOutputsHPFN?UHID=${UHID}&IPID=${IPID}&FromDate=${FromDate}&ToDate=${ToDate}&IsnewVisit=${IsnewVisit}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  FetchPatientSummaryMRDFiles: 'FetchPatientSummaryMRDFiles?PatientID=${PatientID}&AdmissionID=${AdmissionID}&UHID=${UHID}&ImageID=${ImageID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  FetchPatientChiefComplaintAndExaminationsOld: 'FetchPatientChiefComplaintAndExaminationsOld?Admissionid=${Admissionid}&RegCode=${RegCode}&IsnewVisit=${IsnewVisit}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  //FetchLabReportItemlevelGroupPDFN: 'FetchLabReportItemlevelGroupPDFN?RegCode=${RegCode}&TestOrderId=${TestOrderId}&TestOrderItemID=${TestOrderItemID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  FetchPatientAdmissionMedicalReportWorkListPFN: 'FetchPatientAdmissionMedicalReportWorkListPFN?UHID=${UHID}&FromDate=${FromDate}&ToDate=${ToDate}&AdmissionId=${AdmissionId}&TBL=${TBL}&IsNewVisit=${IsNewVisit}&HospitalID=${HospitalID}',
  FetchPatientPhysiotherapyAssessmentOrdersFPFN: 'FetchPatientPhysiotherapyAssessmentOrdersFPFN?UHID=${UHID}&AdmissionID=${AdmissionID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  FetchPatientFinalObgVTEFromPFN: 'FetchPatientFinalObgVTEFromPFN?UHID=${UHID}&AdmissionID=${AdmissionID}&PatientObgVTEID=${PatientObgVTEID}&IsnewVisit=${IsnewVisit}&UserId=${UserId}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  FetchPatientAntenatalVTEForm: 'FetchPatientAntenatalVTEForm?PatientVTEAntenatalID=${PatientVTEAntenatalID}&AdmissionID=${AdmissionID}&UserId=${UserId}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  FetchFinalSaveVTERiskAssessmentPFN: 'FetchFinalSaveVTERiskAssessmentPFN?UHID=${UHID}&AdmissionID=${AdmissionID}&NursingTaskID=${NursingTaskID}&RiskFactorSubGroupID=${RiskFactorSubGroupID}&tbl=${tbl}&IsNewVisit=${IsNewVisit}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  FetchFinalSaveVTESurgicalRiskAssessmentPFN: 'FetchFinalSaveVTERiskAssessmentPFN?UHID=${UHID}&AdmissionID=${AdmissionID}&NursingTaskID=${NursingTaskID}&RiskFactorSubGroupID=${RiskFactorSubGroupID}&tbl=${tbl}&IsNewVisit=${IsNewVisit}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  FetchPatientHandOverformPFN: 'FetchPatientHandOverformPFN?UHID=${UHID}&AdmissionID=${AdmissionID}&HandOverformID=${HandOverformID}&IsnewVisit=${IsnewVisit}&HospitalID=${HospitalID}',
  FetchSSurgeriesPFN: 'FetchSSurgeriesPFN?UHID=${UHID}&AdmissionID=${AdmissionID}&IsnewVisit=${IsnewVisit}&HospitalID=${HospitalID}',
  FetchPatientSickLeaveWorkListPFN: 'FetchPatientSickLeaveWorkListPFN?UHID=${UHID}&FromDate=${FromDate}&ToDate=${ToDate}&HospitalID=${HospitalID}',
  FetchMainProgressNoteNewPFN: 'FetchMainProgressNoteNewPFN?UHID=${UHID}&Admissionid=${Admissionid}&IsnewVisit=${IsnewVisit}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  FetchPainAssessmentOrderItemDetailsPFN: 'FetchPainAssessmentOrderItemDetailsPFN?UHID=${UHID}&AdmissionID=${AdmissionID}&FromDate=${FromDate}&ToDate=${ToDate}&tbl=${tbl}&IsNewVisit=${IsNewVisit}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  FetchDietRequisitionADVPFN: 'FetchDietRequisitionADVPFN?UHID=${UHID}&AdmissionID=${AdmissionID}&IsnewVisit=${IsnewVisit}&UserID=${UserID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
  ...
  FetchFollowUpDetailsPFN: "FetchFollowUpDetailsPFN?UHID=${UHID}&admissionId=${admissionId}&userId=${userId}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}",
  FetchReferalOrdersPFN: "FetchReferalOrdersPFN?UHID=${UHID}&admissionId=${admissionId}&userId=${userId}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}",
  ...
  FetchPatientEyeLensInfo: 'FetchPatientEyeLensInfo?PatientID=${PatientID}&AdmissionID=${AdmissionID}&WorkStationID=${WorkStationID}&HospitalID=${HospitalID}',
};
