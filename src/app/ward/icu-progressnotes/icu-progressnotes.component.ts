...
import { IcuProgressnotesService } from './icu-progressnotes.service';
import { icuDetails } from './urls';
...
@Component({
  selector: 'app-icu-progressnotes',
  templateUrl: './icu-progressnotes.component.html',
  styleUrls: ['./icu-progressnotes.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class IcuProgressnotesComponent extends BaseComponent implements OnInit {
  IsHome = true;
  IsHeadNurse: any;
  currentdate: any;
  patientDetails: any;
  ...
  save() {
    ...
    for (let i = 1; i <= 4; i++) {
      let issue = this.progressNotes.get(`Issue${i}`)?.value || '';
      let plan = this.progressNotes.get(`Plan${i}`)?.value || '';
      issuesAndPlans.push({ [`Issue${i}`]: issue, [`Plan${i}`]: plan });
    }
    ...
    var payload = {
      ...
      "PastMedicalSurgicalHistory": this.progressNotes.get('PastMedicalSurgicalHistory')?.value,
      ...
      "EndorsemenFrom": this.progressNotes.get('EndorsemenFrom')?.value,
      ...
      "Assessment": this.progressNotes.get('Assessment')?.value,
  ...
  FetchPatientICUProgressNoteWorkList() {
    ...
    this.us.get(this.url)
      .subscribe((response: any) => {
        if (response.Code == 200) {
          this.FetchPatientICUProgressNoteWorkListDData = response.FetchPatientICUProgressNoteWorkListDData;
        }
      },
        (err) => {
        ...
        if (response.Code == 200) {
          ...
          if (this.FetchPatientICUProgressNotesDData.length > 0) {
            ...
            this.progressNotes.patchValue({
              ...
              Issue1: IssuesAndPlan[0]?.Issue1,
              Issue2: IssuesAndPlan[1]?.Issue2,
              Issue3: IssuesAndPlan[2]?.Issue3,
              Issue4: IssuesAndPlan[3]?.Issue4,

              Plan1: IssuesAndPlan[0]?.Plan1,
              Plan2: IssuesAndPlan[1]?.Plan2,
              Plan3: IssuesAndPlan[2]?.Plan3,
              Plan4: IssuesAndPlan[3]?.Plan4,
            });
          }
        }
      },
